...
def test_symlinks_manifest():
    ...
    assert read_manifest.file_sums == {
        "foo.txt": "37b51d194a7513e45b56f6524f2d51f2",
...
def test_tree_manifest():
    ...
    assert set(manifest.file_sums.keys()) == {
        "folder/damn.pyc",
        "folder/damn.pyo",
        "one.ext",
        "path/to/two.txt",
...
def test_already_pyc_in_manifest():
    ...
    # Not included the pycs or pyo
    assert set(read_manifest.file_sums.keys()) == {
        "conanfile.py",
        "conanfile.pyc",
        "conanfile.pyo",
    }
...
def test_pycache_included():
    tmp_dir = temp_folder()
    files = {"__pycache__/damn.py": "binarythings", "pythonfile.pyc": "binarythings3"}
    for filename, content in files.items():
        save(os.path.join(tmp_dir, filename), content)
