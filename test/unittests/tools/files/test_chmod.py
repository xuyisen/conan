...
@pytest.mark.skipif(
    platform.system() == "Windows", reason="validate full permissions only in Unix"
)
@pytest.mark.parametrize(
    "read,write,execute,expected",
    [
...
def test_chmod_single_file(read, write, execute, expected):
    ...
    chmod(
        conanfile, file_path, read=read, write=write, execute=execute, recursive=False
    )
...
def test_chmod_recursive(read, write, execute, expected):
    ...
    conanfile = ConanFileMock()
    chmod(
        conanfile, folder_path, read=read, write=write, execute=execute, recursive=True
    )
    for file in files.keys():
...
@pytest.mark.skipif(
    platform.system() == "Windows", reason="Validate default permissions only in Unix"
)
...
def test_missing_permission_arguments():
    ...
    assert (
        "Could not change permission: At least one of the permissions should be set."
        in str(error.value)
    )
...
def test_invalid_path():
    ...
    assert 'Could not change permission: Path "invalid_path" does not exist.' in str(
        error.value
    )
...
@pytest.mark.skipif(
    ...
    reason="Validate read-only permissions only in Windows",
)
