...
def test_source_folder_works(chdir_mock):
    ...
    os.chdir(folder)
    save_toolchain_args(
        {"configure_args": "-foo bar", "make_args": "", "autoreconf_args": "-bar foo"}
    )
    ...
    autotools.configure()
    assert (
        conanfile.command.replace("\\", "/") == '"/path/to/sources/configure" -foo bar '
    )

    autotools.autoreconf(build_script_folder="subfolder")
    chdir_mock.assert_called_with(
        autotools, os.path.normpath(os.path.join("/path/to/sources", "subfolder"))
    )
