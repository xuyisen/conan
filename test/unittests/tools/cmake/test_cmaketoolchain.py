...
def test_enabled_blocks_conf(conanfile):
    ...
    with pytest.raises(ConanException) as e:
        _ = toolchain.content
    assert (
        "Block 'potato' defined in tools.cmake.cmaketoolchain:enabled_blocks doesn't"
        in str(e)
    )
...
def test_context_update(conanfile):
    ...
    toolchain.blocks["generic_system"].values["cmake_system_name"] = (
        "Super" + cmake_system_name
    )
    content = toolchain.content
...
def test_toolset(conanfile_msvc):
    ...
    assert (
        'set(CMAKE_GENERATOR_TOOLSET "v143" CACHE STRING "" FORCE)' in toolchain.content
    )
...
def test_toolset_update_version(conanfile_msvc):
    ...
    toolchain = CMakeToolchain(conanfile_msvc)
    assert (
        'set(CMAKE_GENERATOR_TOOLSET "v143,version=14.38" CACHE STRING "" FORCE)'
        in toolchain.content
    )
...
def test_toolset_update_version_conf(conanfile_msvc):
    ...
    toolchain = CMakeToolchain(conanfile_msvc)
    assert (
        'set(CMAKE_GENERATOR_TOOLSET "v143,version=14.37" CACHE STRING "" FORCE)'
        in toolchain.content
    )
...
def test_toolset_update_version_overflow(conanfile_msvc):
    ...
    toolchain = CMakeToolchain(conanfile_msvc)
    assert (
        'set(CMAKE_GENERATOR_TOOLSET "v143,version=14.48" CACHE STRING "" FORCE)'
        in toolchain.content
    )
...
def test_toolset_x64(conanfile_msvc):
    ...
    toolchain = CMakeToolchain(conanfile_msvc)
    assert (
        'set(CMAKE_GENERATOR_TOOLSET "v143,host=x64" CACHE STRING "" FORCE)'
        in toolchain.content
    )
    assert "Visual Studio 17 2022" in toolchain.generator
    assert "CMAKE_CXX_STANDARD 20" in toolchain.content
...
def test_toolset_cuda(conanfile_msvc):
    conanfile_msvc.conf.define(
        "tools.cmake.cmaketoolchain:toolset_cuda", "C:/Path/To/CUDA"
    )
    toolchain = CMakeToolchain(conanfile_msvc)
    assert (
        'set(CMAKE_GENERATOR_TOOLSET "v143,cuda=C:/Path/To/CUDA" CACHE STRING "" FORCE)'
        in toolchain.content
    )
    ...
    c.settings = Settings(
        {
            "os": ["Windows"],
            "compiler": {
                "msvc": {"version": ["170"], "update": [None], "cppstd": ["98"]}
            },
            "build_type": ["Release"],
            "arch": ["x86"],
        }
    ...
    c.settings = Settings(
        {
            "os": ["Windows"],
            "compiler": {
                "msvc": {"version": ["192"], "update": [8, 9], "cppstd": ["14"]}
            },
            "build_type": ["Release"],
            "arch": ["x86_64"],
        }
...
def conanfile_linux_shared():
    c = ConanFile()
    c.options = Options(
        {"fPIC": [True, False], "shared": [True, False]},
        {
            "fPIC": False,
            "shared": True,
        },
...
def test_fpic_when_shared_true(conanfile_linux_shared, fPIC):
    ...
    content = toolchain.content
    assert (
        "set(CMAKE_POSITION_INDEPENDENT_CODE {} CACHE BOOL".format(cmake_value)
        in content
    )
...
def conanfile_windows_fpic():
    ...
    c.options = Options({"fPIC": [True, False]}, {"fPIC": True})
    c.settings = Settings(
        {
            "os": ["Windows"],
...
@pytest.mark.parametrize(
    "os,arch,expected_sdk",
    [("iOS", "x86_64", ""), ("watchOS", "armv8", ""), ("tvOS", "x86_64", "")],
)
...
def test_linker_scripts_block(conanfile):
    ...
    content = toolchain.content
    assert (
        r'string(APPEND CONAN_EXE_LINKER_FLAGS " -T\"path_to_first_linker_script\" '
