...
def test_macos_not_supported_for_new_compilers(mode):
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings(
        {"compiler.version": "2021.3", "compiler.mode": mode, "os": "Darwin"}
    )
...
def test_error_if_detected_intel_legacy_version(os_):
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings(
        {"compiler.version": "19.1", "compiler.mode": "classic", "os": os_}
    )
    with pytest.raises(ConanException) as excinfo:
        IntelCC(conanfile)
...
def test_check_ms_toolsets(mode, expected):
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings(
        {"compiler.version": "2021.3", "compiler.mode": mode, "os": "Windows"}
    )
...
def test_installation_path_in_conf():
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings(
        {"compiler.version": "2021.3", "compiler.mode": "classic", "os": "Windows"}
    )
...
@pytest.mark.parametrize(
    "os_,call_command,setvars_file",
    [("Windows", "call", "setvars.bat"), ("Linux", ".", "setvars.sh")],
)
@patch("conan.tools.intel.intel_cc.platform.system")
def test_setvars_command_with_custom_arguments(
    platform_system, os_, call_command, setvars_file
):
    platform_system.return_value = os_
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings(
        {"compiler.version": "2021.3", "compiler.mode": "icx", "os": os_}
    )
    fake_path = "mysuper/path/to/intel/oneapi"
    ...
    expected = '%s "%s" %s' % (
        call_command,
        os.path.join(fake_path, setvars_file),
        args,
    )
