...
def test_invalid_compiler():
    ...
    conanfile.settings = MockSettings(
        {"compiler": "Visual Studio", "compiler.version": "20220203"}
    )
    assert msvs_toolset(conanfile) is None


@pytest.mark.parametrize(
    "compiler_version,expected_toolset",
    [
        ("194", "v143"),
        ("193", "v143"),
        ("192", "v142"),
...
def test_msvc_default(compiler_version, expected_toolset):
    ...
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings(
        {"compiler": "msvc", "compiler.version": compiler_version}
    )
...
def test_intel_cc_old_compiler():
    ...
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings(
        {"compiler": "intel-cc", "compiler.version": "19.1"}
    )
...
@pytest.mark.parametrize(
    "compiler_version,compiler_mode,expected_toolset",
    [
...
def test_clang_platform_toolset():
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings(
        {"compiler": "clang", "compiler.version": "17", "compiler.runtime": "dynamic"}
    )
