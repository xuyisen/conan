...
class TestHashes:
    def test_md5(self):
        ...
        check_sha1(
            ConanFileMock(), filepath, "eb599ec83d383f0f25691c184f656d40384f9435"
        )
        check_sha256(
            ConanFileMock(),
            filepath,
        ...
        with pytest.raises(
            ConanException, match="md5 signature failed for 'file.txt' file."
        ):
            check_md5(ConanFileMock(), filepath, "invalid")

        with pytest.raises(
            ConanException, match="sha1 signature failed for 'file.txt' file."
        ):
            check_sha1(ConanFileMock(), filepath, "invalid")

        with pytest.raises(
            ConanException, match="sha256 signature failed for 'file.txt' file."
        ):
