...
class TestDetectedArchitecture:
    ...
    def test_various(self, mocked_machine, expected_arch):
        with mock.patch(
            "platform.machine", mock.MagicMock(return_value=mocked_machine)
        ):
            assert expected_arch == detect_arch(), (
                f"given '{mocked_machine}' expected '{expected_arch}'"
            )

    def test_aix(self):
        with (
            mock.patch("platform.machine", mock.MagicMock(return_value="00FB91F44C00")),
            ...
            mock.patch(
                "conan.internal.api.detect.detect_api._get_aix_conf",
                mock.MagicMock(return_value="32"),
            ),
            mock.patch(
                "subprocess.check_output", mock.MagicMock(return_value="7.1.0.0")
            ),
        ):
        ...
        with (
            ...
            mock.patch(
                "conan.internal.api.detect.detect_api._get_aix_conf",
                mock.MagicMock(return_value="64"),
            ),
            mock.patch(
                "subprocess.check_output", mock.MagicMock(return_value="7.1.0.0")
            ),
    ...
    def test_solaris(self):
        with (
            mock.patch("platform.machine", mock.MagicMock(return_value="sun4v")),
            ...
            mock.patch(
                "platform.architecture", mock.MagicMock(return_value=("64bit", "ELF"))
            ),
        ...
        with (
            mock.patch("platform.machine", mock.MagicMock(return_value="i86pc")),
    ...
    def test_e2k(self, processor, expected_arch):
        with (
            mock.patch("platform.machine", mock.MagicMock(return_value="e2k")),
