...
class TestGetCasedPath:
    ...
    def test_case_not_existing(self):
        ...
        non_existing_path = os.path.join(
            current_dir, "this", "Path", "does", "NOT", "Exists"
        )
        p = get_cased_path(
            non_existing_path
        )  # If not exists from the root, returns as is
    ...
    def test_case_partial_exists(self):
        ...
        non_existing_path = os.path.join(
            folder, "myfolder", "subfolder", "not-existing"
        )
...
class TestUnixPath:
    ...
    def test_cygwin_path(self):
        assert "/cygdrive/c/windows/system32" == subsystem_path(
            "cygwin", "C:\\Windows\\System32"
        )
        ...
        assert "../configure" == subsystem_path("cygwin", "../configure")
        assert "source_subfolder/configure" == subsystem_path(
            "cygwin", "source_subfolder/configure"
        )
    ...
    def test_wsl_path(self):
        assert "/mnt/c/Windows/System32" == subsystem_path(
