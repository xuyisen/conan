...
def test_configure_arguments():
    ...
    os.chdir(tmp)
    save_toolchain_args(
        {"configure_args": "my_configure_args", "make_args": "my_make_args"}
    )
    runner = RunnerMock()
    conanfile = ConanFile()
    ...
    assert (
        "my_make install_other my_make_args DESTDIR=None -j23" == runner.command_called
    )

    for make_args in ["my_make_args", ""]:
        save_toolchain_args(
            {"configure_args": "my_configure_args", "make_args": f"{make_args}"}
        )
        ...
        assert "-j23" not in runner.command_called
        assert (
            f"my_make install{make_args_str} DESTDIR=None -j1" == runner.command_called
        )

        ab.install(args=["DESTDIR=whatever", "-j1"])
        assert "-j23" not in runner.command_called
        assert (
            f"my_make install{make_args_str} DESTDIR=whatever -j1"
        ...
        assert "-j23" not in runner.command_called
        assert (
            f"my_make install{make_args_str} DESTDIR=whatever -arg1 -j1 -arg2"
            == runner.command_called
        )
        ...
        assert (
            f"my_make install{make_args_str} DESTDIR=/user/smith-john/whatever -j23"
            == runner.command_called
        )
        ...
        assert (
            f"my_make install{make_args_str} DESTDIR=/user/smith-j47/whatever -j23"
            == runner.command_called
        )
