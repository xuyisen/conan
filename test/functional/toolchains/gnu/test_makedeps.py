...
def test_make_deps_definitions_escape():
    ...
    client.run_command(
        f"make --print-data-base -f {CONAN_MAKEFILE_FILENAME}", assert_error=True
    )
    assert r"CONAN_CXXFLAGS_HELLO = flag2=\"my flag2\"" in client.out
    assert r"CONAN_CFLAGS_HELLO = flag1=\"my flag1\"" in client.out
    assert (
        r'CONAN_DEFINES_HELLO = $(CONAN_DEFINE_FLAG)USER_CONFIG="user_config.h" $(CONAN_DEFINE_FLAG)OTHER="other.h"'
        in client.out
    )
...
def test_makedeps_with_makefile_build():
    ...
    with client.chdir("lib"):
        client.save(
            {
                "Makefile": gen_makefile(libs=["hello"]),
                "hello.cpp": gen_function_cpp(
                    name="hello", includes=["hello"], calls=["hello"]
                ),
        ...
        client.run(
            "create . --name=hello --version=0.1.0 -c tools.microsoft.bash:subsystem=msys2 -c tools.microsoft.bash:path=bash"
        )
    with client.chdir("global"):
        # Consume from global variables
        client.run(
            "install --requires=hello/0.1.0 -pr:b=default -pr:h=default -g MakeDeps -of build"
        )
        client.save(
            {
