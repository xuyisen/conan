...
class TestPkgConfig:
    ...
    def test_negative(self):
        ...
        assert (
            "PkgConfig failed. Command: pkg-config --libs-only-l something_that_not_exist "
...
def test_pkg_config_round_tripe_cpp_info():
    ...
    try:
        ...
        os_ = platform.system()
        pkg_config_path = (
            tools_locations["pkg_config"][version]["path"][os_] + "/" + exe
        )
    except KeyError:
    ...
    # paths
    assert (
        f'set(pkg_INCLUDE_DIRS_NONE "{prefix}/usr/local/include/libastral")' in pkg_data
    )
