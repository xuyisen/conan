...
def test_xcodedeps_build_configurations():
    ...
    for config in ["Release", "Debug"]:
        client.run(
            "install . -s build_type={} -s arch=x86_64 --build=missing -g XcodeDeps".format(
                config
            )
        )
    ...
    for config in ["Release", "Debug"]:
        client.run_command(
            "xcodebuild -project app.xcodeproj -configuration {} -arch x86_64".format(
                config
            )
        )
    ...
    client.save(
        {
            ...
            .with_package_info(cpp_info={"frameworks": ["CoreFoundation"]}, env_info={})
        }
    )
...
def test_xcodedeps_dashes_names_and_arch():
    ...
    client = TestClient(path_with_spaces=False)
    client.save(
        {"conanfile.py": GenConanfile().with_name("hello-dashes").with_version("0.1")}
    )
    client.run("export .")
