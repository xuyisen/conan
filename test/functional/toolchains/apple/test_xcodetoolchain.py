...
@pytest.mark.parametrize(
    "cppstd, cppstd_output, min_version",
    [
...
def test_project_xcodetoolchain(cppstd, cppstd_output, min_version):
    ...
    sdk_version = "14.5"
    settings = (
        "-s arch=x86_64 -s os.sdk_version={} -s compiler.cppstd={} "
        "-s compiler.libcxx=libc++ -s os.version={} "
        "-c tools.build.cross_building:can_run=True "
        "-c 'tools.build:cflags=[\"-fstack-protector-strong\"]'".format(
            sdk_version, cppstd, min_version
        )
    )
