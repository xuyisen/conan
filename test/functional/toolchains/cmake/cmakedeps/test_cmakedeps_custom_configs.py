...
class CustomConfigurationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client.run(
            "create . -s compiler.version=191 "
            '-s build_type=Release -o hello/*:shared=True -tf=""'
        )
        self.client.run(
            "create . --name=hello --version=0.1 -s compiler.version=191 "
    ...
    def test_generator_multi(self):
        ...
        with self.client.chdir("build"):
            ...
            self.assertTrue(
                os.path.isfile(
                    os.path.join(
                        self.client.current_folder, "hello-Target-releaseshared.cmake"
                    )
                )
            )
            self.assertTrue(
                os.path.isfile(
                    os.path.join(
                        self.client.current_folder, "hello-Target-release.cmake"
                    )
                )
            )

            self.client.run_command(
                'cmake .. -G "Visual Studio 15 Win64" --loglevel=DEBUG'
            )
...
class CustomSettingsTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cmake = cmake.replace(
            "PUBLIC_HEADER", "CONFIGURATIONS MyRelease\nPUBLIC_HEADER"
        )
        self.client.save({"CMakeLists.txt": cmake})
        self.client.run(
            "create . --name=hello --version=0.1 -s compiler.version=191 -s build_type=MyRelease "
    ...
    def test_generator_multi(self):
        ...
        with self.client.chdir(build_directory):
            ...
            self.assertTrue(
                os.path.isfile(
                    os.path.join(
                        self.client.current_folder, "hello-Target-myrelease.cmake"
                    )
                )
            )
...
def test_changing_build_type():
    ...
    client.save(
        {"conanfile.py": pkg_conanfile, "CMakeLists.txt": cmakelists, "app.cpp": app},
        clean_first=True,
    )
    ...
    cmd = (
        os.path.join(".", "Debug", "app") if platform.system() == "Windows" else "./app"
    )
