...
def test_no_soname_flag(nosoname_property):
    ...
    replace_in_file(
        ConanFileMock(),
        os.path.join(client.current_folder, "CMakeLists.txt"),
    ...
    client.run(
        "install --requires=app/0.1@ -o nosoname*:shared=True -o lib_b/*:shared=True"
    )
    ...
    if nosoname_property is False:
        ...
        assert (
            "libnosoname.so: cannot open shared object file: "
