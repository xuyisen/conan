...
def _add_message_status_flags(client):
    ...
    with open(cmakelists_path, "a") as cmakelists_file:
        ...
        cmakelists_file.write(
            'message(STATUS "CONAN_OBJC_FLAGS: ${CONAN_OBJC_FLAGS}")\n'
        )
        cmakelists_file.write(
            'message(STATUS "CONAN_OBJCXX_FLAGS: ${CONAN_OBJCXX_FLAGS}")\n'
        )
...
@pytest.mark.parametrize(
    ...
    [("watchOS", "8.1", "watchos", "armv7k"), ("tvOS", "13.2", "appletvos", "armv8")],
)
def test_cmake_apple_bitcode_arc_and_visibility_flags_enabled(
    op_system, os_version, sdk, arch
):
    ...
    toolchain = client.load(
        os.path.join("build", "Release", "generators", "conan_toolchain.cmake")
    )
    # bitcode
    ...
    assert (
        "-- CONAN_OBJC_FLAGS:  -fembed-bitcode -fvisibility=default -fobjc-arc"
        in client.out
    )
    assert (
        "-- CONAN_OBJCXX_FLAGS:  -fembed-bitcode -fvisibility=default -fobjc-arc"
        in client.out
    )
...
def test_cmake_apple_bitcode_arc_and_visibility_flags_enabled_and_xcode_generator(
    op_system, os_version, sdk, arch
):
    """
    ...
    _add_message_status_flags(client)
    client.run(
        'create . --profile:build=default --profile:host=host -tf="" '
...
def test_cmake_apple_bitcode_arc_and_visibility_flags_disabled(
    op_system, os_version, sdk, arch
):
    profile = textwrap.dedent(
        """
    ...
    assert (
        'set(CMAKE_XCODE_ATTRIBUTE_BITCODE_GENERATION_MODE "bitcode")' not in toolchain
    )
    ...
    assert (
        'set(VISIBILITY "-fvisibility=hidden -fvisibility-inlines-hidden")' in toolchain
    )

    client.run('create . --profile:build=default --profile:host=host -tf=""')
    # flags
    assert (
        "-- CONAN_C_FLAGS:   -fvisibility=hidden -fvisibility-inlines-hidden"
        in client.out
    )
    assert (
        "-- CONAN_CXX_FLAGS:   -fvisibility=hidden -fvisibility-inlines-hidden"
        in client.out
    )
    assert (
        "-- CONAN_OBJC_FLAGS:   -fvisibility=hidden -fvisibility-inlines-hidden -fno-objc-arc"
        in client.out
    )
    assert (
        "-- CONAN_OBJCXX_FLAGS:   -fvisibility=hidden -fvisibility-inlines-hidden -fno-objc-arc"
        in client.out
    )
    assert "[100%] Built target hello" in client.out
...
def test_cmake_apple_bitcode_arc_and_visibility_flags_are_none(
    op_system, os_version, sdk, arch
):
    """
