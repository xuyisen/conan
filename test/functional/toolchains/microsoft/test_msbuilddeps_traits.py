...
def test_transitive_headers_not_public(transitive_libraries):
    ...
    assert (
        "<ConanmatrixIncludeDirectories></ConanmatrixIncludeDirectories>" in matrix_data
    )
    assert (
        "<ConanmatrixLibraryDirectories>$(ConanmatrixRootFolder)/lib;"
...
def test_shared_requires_static(transitive_libraries):
    ...
    assert not os.path.exists(
        os.path.join(c.current_folder, "conan_matrix_vars_release_x64.props")
    )
