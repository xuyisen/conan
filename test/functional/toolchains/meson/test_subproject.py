...
def test_subproject():
    client = TestClient()
    test_package_c = gen_function_c(
        name="main", includes=["greeter"], calls=["greeter"]
    )
    client.save(
        {
            ...
            os.path.join(
                "subprojects", "hello", "meson.build"
            ): _meson_subproject_build,
            os.path.join(
                "subprojects", "hello", "meson_options.txt"
            ): _meson_subproject_options,
