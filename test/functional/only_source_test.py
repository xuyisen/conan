...
class OnlySourceTest(unittest.TestCase):
    def test_conan_test(self):
        ...
        client.save(
            {
                "conanfile.py": GenConanfile("hello1", "1.1").with_require(
                    "hello0/0.0@lasote/stable"
                )
            }
        )
        client.run("export . --user=lasote --channel=stable")
        ...
        client.save(
            {
                "conanfile.py": GenConanfile("hello2", "2.2").with_require(
                    "hello1/1.1@lasote/stable"
                ),
        ...
        self.assertIn(
            "Try to build locally from sources using the '--build=hello0/0.0@lasote/stable "
            "--build=hello1/1.1@lasote/stable'",
            client.out,
        )
        # Only 1 reference!
        assert (
            "List all available packages using 'conan list \"hello0/0.0@lasote/stable:*\" -r=remote'"
            in client.out
        )
        ...
        client.run("create . --user=lasote --channel=stable", assert_error=True)
        self.assertIn(
            "Try to build locally from sources using the "
            "'--build=hello1/1.1@lasote/stable'",
            client.out,
        )
    ...
    def test_build_policies_in_conanfile(self):
        ...
        self.assertIn(
            "ERROR: hello0/1.0@lasote/stable: build_policy='always' has been removed",
            client.out,
        )
    ...
    def test_reuse(self):
        ...
        self.assertFalse(
            os.path.exists(other_client.get_latest_pkg_layout(pref).build())
        )
        self.assertTrue(
            os.path.exists(other_client.get_latest_pkg_layout(pref).package())
        )
        ...
        other_client = TestClient(servers=client.servers)
        other_client.run(
            "install --requires=%s --build HelloInvalid -b hello" % str(ref)
        )
        # self.assertIn("No package matching 'HelloInvalid' pattern", other_client.out)
...
def test_build_policy_missing():
    ...
    conanfile = (
        GenConanfile("pkg", "1.0")
        .with_class_attribute('build_policy = "missing"')
    ...
    assert (
        "pkg/1.0: Building package from source as defined by build_policy='missing'"
        in c.out
    )
