...
def test_apt_install_substitutes():
    ...
    client.run(
        "create . --name=test --version=1.0 -c tools.system.package_manager:mode=install "
...
def test_build_require():
    ...
    client.save(
        {
            "tool_require.py": textwrap.dedent("""
        from conan import ConanFile
        from conan.tools.system.package_manager import Apt
    ...
    client.run(
        "create consumer.py --name=consumer --version=1.0 "
...
def test_brew_install_check_mode():
    ...
    assert (
        "System requirements: 'non-existing1, non-existing2' are missing but "
...
def test_brew_install_install_mode():
    ...
    client.run(
        "create . test/1.0@ -c tools.system.package_manager:mode=install",
        assert_error=True,
    )
...
def test_collect_system_requirements():
    ...
    with patch.object(
        _SystemPackageManagerTool, "_conanfile_run", MagicMock(return_value=False)
    ):
        client.run(
            "install . -c tools.system.package_manager:tool=apt-get --format=json",
            redirect_stdout="graph.json",
        )
    graph = json.loads(client.load("graph.json"))
    assert {"apt-get": {"install": ["pkg1", "pkg2"], "missing": []}} == graph["graph"][
        "nodes"
    ]["0"]["system_requires"]

    # plain report, do not check
    client.run(
        "graph info . -c tools.system.package_manager:tool=apt-get "
    ...
    assert {"apt-get": {"install": ["pkg1", "pkg2"]}} == graph2["graph"]["nodes"]["0"][
        "system_requires"
    ]

    # Check report-installed
    with patch.object(
        _SystemPackageManagerTool, "_conanfile_run", MagicMock(return_value=True)
    ):
        client.run(
            "graph info . -c tools.system.package_manager:tool=apt-get "
        ...
        graph2 = json.loads(client.load("graph2.json"))
        assert {
            "apt-get": {"install": ["pkg1", "pkg2"], "missing": ["pkg1", "pkg2"]}
    ...
    assert (
        "ERROR: conanfile.py: Error in system_requirements() method, line 11"
        in client.out
    )
