...
@pytest.mark.parametrize("old_args", ["-op=v1 -or=pkg/1.0", "-or=pkg/1.0"])
...
@pytest.mark.parametrize(
    "formatter", ["-f=json --out-file=output.json", "-f=html --out-file=output.html"]
)
...
def test_compare_paths(old_args, new_args, formatter):
    ...
    if "json" in formatter:
        ...
        assert any(
            os.path.join(v1_path, "conanfile.py").replace("\\", "/") in k
            for k in output_json.keys()
        )
        assert any(
            os.path.join(v1_path, "conanmanifest.txt").replace("\\", "/") in k
            for k in output_json.keys()
        )
        # We have patch information
        assert any(
            os.path.abspath(
                os.path.join(v1_path, "..", "es", "patches", "patch.patch")
            ).replace("\\", "/")
            in k
            for k in output_json.keys()
        )
        ...
        assert any(
            os.path.abspath(os.path.join(v1_path, "..", "e", "v1.txt")).replace(
                "\\", "/"
            )
            in k
            for k in output_json.keys()
