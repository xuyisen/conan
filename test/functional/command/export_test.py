...
class TestRevisionModeSCM:
    def test_revision_mode_scm(self):
        ...
        assert (
            "Can't have a dirty repository using revision_mode='scm' and doing" in t.out
        )
        # Commit to fix
    ...
    def test_revision_mode_scm_subfolder(self):
        ...
        t = TestClient()
        conanfile = str(
            GenConanfile().with_class_attribute('revision_mode = "scm_folder"')
        )
        commit = t.init_git_repo(
            {"pkga/conanfile.py": conanfile, "pkgb/conanfile.py": conanfile}
        )

        t.save({"pkgb/conanfile.py": conanfile + "\n#comment"})
        commit_b = git_add_changes_commit(
            os.path.join(t.current_folder, "pkgb"), msg="fix"
        )
    ...
    def test_revision_mode_scm_excluded_files(self, conf_excluded, recipe_excluded):
        t = TestClient()
        recipe_excluded = (
            f"revision_mode_excluded = {recipe_excluded}" if recipe_excluded else ""
        )
        ...
        conanfile = (
            ...
            .with_class_attribute('revision_mode = "scm"')
            .with_class_attribute(recipe_excluded)
        )
        commit = t.init_git_repo({"conanfile.py": str(conanfile), "test.cpp": "mytest"})

        t.run("export .")
        assert t.exported_recipe_revision() == commit

        t.save({"test.cpp": "mytest2", "new.txt": "new", "src/potato": "hello"})
        t.run("export . -vvv")
        assert t.exported_recipe_revision() == commit
