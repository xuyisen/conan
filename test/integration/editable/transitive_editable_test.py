...
def test_transitive_editables_python_requires_version_range():
    ...
    assert (
        "Version range '*' from requirement 'dep/[*]' required by 'python_requires' "
    ...
    client.run(
        "editable add pkg"
    )  # now it works, able to return from server if necessary
...
def test_transitive_editable_cascade_build():
    ...
    pkgb = (
        GenConanfile("pkgb", "1.0")
        .with_requires("pkga/1.0")
        .with_package_type("static-library")
    )
    pkgc = (
        GenConanfile("pkgc", "1.0")
        .with_requires("pkgb/1.0")
        .with_package_type("static-library")
    )
    app = (
        GenConanfile("app", "1.0")
        .with_requires("pkgc/1.0")
    ...
    c.assert_listed_binary(
        {"pkgb/1.0": (pkgb_id, "Cache"), "pkgc/1.0": (pkgc_id, "Cache")}
    )
    ...
    c.assert_listed_binary(
        {"pkgb/1.0": (pkgb_id, "Build"), "pkgc/1.0": (pkgc_id, "Build")}
    )
...
def test_transitive_editable_cascade_package_id():
    ...
    # The consumers didn't need a new binary, even if I modified pkg
    c.assert_listed_binary(
        {"pkgb/1.0": (pkgb_id, "Cache"), "pkgc/1.0": (pkgc_id, "Build")}
    )
...
def test_warning_from_cache():
    ...
    c.run("create pkgb")
    assert (
        "pkgb/1.0: WARN: risk: Package is being built in the cache using editable"
        in c.out
    )
    c.run("create pkgc")
    assert (
        "pkgc/1.0: WARN: risk: Package is being built in the cache using editable"
        in c.out
    )
