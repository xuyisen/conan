...
def test_upload_parallel_error():
    ...
    client = TestClient(
        requester_class=FailOnReferencesUploader, default_server_user=True
    )
    client.save_home(
        {"global.conf": "core.upload:parallel=2\ncore.upload:retry_wait=0"}
    )
    client.save({"conanfile.py": GenConanfile()})
    client.run("remote login default admin -p password")
    for index in range(4):
        client.run(
            "create . --name=lib{} --version=1.0 --user=user --channel=channel".format(
                index
            )
        )
...
def test_upload_parallel_success():
    ...
    client.save_home({"global.conf": "core.upload:parallel=2"})
    client.save({"conanfile.py": GenConanfile()})
    ...
    assert (
        "Uploading recipe 'lib0/1.0@user/channel#4d670581ccb765839f2239cc8dff8fbd'"
        in client.out
    )
    assert (
        "Uploading recipe 'lib1/1.0@user/channel#4d670581ccb765839f2239cc8dff8fbd'"
        in client.out
    )
