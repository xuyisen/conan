...
class TestUser:
    def test_command_user_no_remotes(self):
        ...
        assert (
            "ERROR: Remote 'wrong_remote' can't be found or is disabled" in client.out
        )
    ...
    def test_command_user_list(self):
        ...
        assert (
            "ERROR: Remote 'Test_Wrong_Remote' can't be found or is disabled"
            in client.out
        )
    ...
    def test_with_remote_no_connect(self):
        ...
        assert (
            "Changed user of remote 'default' from 'None' (anonymous) to 'john'"
            in client.out
        )
        ...
        assert (
            "Changed user of remote 'default' from 'john' (anonymous) to 'will'"
            in client.out
        )
        ...
        assert (
            "Changed user of remote 'default' from 'will' (anonymous) to 'None' (anonymous)"
            in client.out
        )
    ...
    def test_command_user_with_password(self):
        ...
        assert (
            "Changed user of remote 'default' from 'None' (anonymous) to 'admin'"
            in client.out
        )
        client.run("remote logout default")
        assert (
            "Changed user of remote 'default' from 'admin' (authenticated) to 'None' (anonymous)"
            in client.out
        )
        localdb = LocalDB(client.cache_folder)
        assert (None, None, None) == localdb.get_login(test_server.fake_url)
    ...
    def test_command_user_with_password_spaces(self):
        ...
        assert (
            "Changed user of remote 'default' from 'None' (anonymous) to 'lasote'"
            in client.out
        )
        client.run("remote logout default")
        client.run(r'remote login default lasote -p "my \"password"')
...
"""
    ...
    def test_authenticated(self):
        ...
        client = TestClient(
            servers=servers, inputs=["lasote", "mypass", "mypass", "mypass"]
        )
        client.run("remote logout default")
        assert (
            "Changed user of remote 'default' from 'None' (anonymous) to 'None' (anonymous)"
            in client.out
        )
        ...
        assert (
            "Changed user of remote 'default' from 'lasote' (authenticated) to 'danimtb' (authenticated)"
            in client.out
        )
        client.run("remote list-users")
        assert "default:\n  Username: danimtb\n  authenticated: True" in client.out
    ...
    def test_json(self):
        ...
        client = TestClient(
            servers=servers, inputs=["lasote", "mypass", "danimtb", "passpass"]
        )
        client.run("remote list-users -f json")
        info = json.loads(client.stdout)
        assert info == [
            {"name": "default", "authenticated": False, "user_name": None},
            {"name": "other_server", "authenticated": False, "user_name": None},
        ]
        ...
        info = json.loads(client.stdout)
        assert info == [
            {"name": "default", "authenticated": False, "user_name": "bad_user"},
            {"name": "other_server", "authenticated": False, "user_name": None},
        ]
        ...
        info = json.loads(client.stdout)
        assert info == [
            {"name": "default", "authenticated": False, "user_name": "lasote"},
            {"name": "other_server", "authenticated": False, "user_name": None},
        ]
        ...
        info = json.loads(client.stdout)
        assert info == [
            {"name": "default", "authenticated": True, "user_name": "lasote"},
            {"name": "other_server", "authenticated": False, "user_name": None},
        ]
        ...
        info = json.loads(client.stdout)
        assert info == [
            {"name": "default", "authenticated": True, "user_name": "danimtb"},
            {"name": "other_server", "authenticated": False, "user_name": None},
        ]
        ...
        assert info == [
            {"name": "default", "authenticated": True, "user_name": "danimtb"},
            {"name": "other_server", "authenticated": False, "user_name": "lasote"},
        ]
        ...
        info = json.loads(client.stdout)
        assert info == [
            {"name": "default", "authenticated": False, "user_name": "danimtb"},
            {"name": "other_server", "authenticated": False, "user_name": None},
        ]
        client.run("remote list-users")
        assert "default:\n  Username: danimtb\n  authenticated: False" in client.out
    ...
    def test_skip_auth(self):
        ...
        client.run("remote set-user default lasote")
        assert (
            "Changed user of remote 'default' from 'lasote' (authenticated) to 'lasote' (authenticated)"
            in client.out
        )
        ...
        assert (
            "Changed user of remote 'default' from 'lasote' (authenticated) to 'flanders' (anonymous)"
            in client.out
        )
...
class TestRemoteAuth:
    def test_remote_auth(self):
        servers = OrderedDict()
        servers["default"] = TestServer(
            users={"lasote": "mypass", "danimtb": "passpass"}
        )
    ...
    def test_remote_auth_force(self):
        ...
        with patch(
            "conan.internal.rest.rest_client_v2.RestV2Methods.check_credentials"
    ...
    def test_remote_auth_with_user_env_var(self):
        ...
        with environment_update(
            {
                "CONAN_LOGIN_USERNAME_DEFAULT": "lasote",
    ...
    def test_remote_auth_error(self):
        ...
        c = TestClient(
            servers=servers, inputs=["user1", "pass", "user2", "pass", "user3", "pass"]
        )
        c.run("remote auth *")
    ...
    def test_remote_auth_server_expire_token_secret(self):
        ...
        c.run("remote auth *")
        assert (
            "Remote 'default' needs authentication, obtaining credentials" not in c.out
        )
        assert "user: myuser" in c.out
    ...
    def test_remote_auth_server_expire_token(self):
        server = TestServer(users={"myuser": "password", "myotheruser": "otherpass"})
        server.test_server.ra.api_v2.credentials_manager.expire_time = timedelta(
            seconds=2
        )
        c = TestClient(
            servers={"default": server},
    ...
    def test_auth_after_logout(self):
        ...
        c.run("remote logout *")
        assert (
            "Changed user of remote 'default' from 'myuser' (authenticated) to 'None'"
            in c.out
        )
        c.run("remote auth *")
        assert "Remote 'default' needs authentication, obtaining credentials" in c.out
