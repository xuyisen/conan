...
class InstallMissingDependency(unittest.TestCase):
    def test_missing_dep(self):
        ...
        dep2_conanfile = GenConanfile("dep2", "1.0").with_require(
        ...
        #   \ -> dep2/1.0---->/
        conanfile = (
            GenConanfile("foo", "1.0")
            .with_require("dep1/1.0@lasote/testing")
        ...
        conanfile = (
            GenConanfile("foo", "1.0")
            .with_requirement("dep1/2.0@lasote/testing", force=True)
            .with_require("dep2/1.0@lasote/testing")
        )
        client.save({"conanfile.py": conanfile}, clean_first=True)
        client.run("create . --user=lasote --channel=testing", assert_error=True)
        client.assert_overrides(
            {"dep1/1.0@lasote/testing": ["dep1/2.0@lasote/testing"]}
        )
    ...
    def test_missing_multiple_dep(self):
        ...
        self.assertIn(
            "ERROR: Missing prebuilt package for 'dep1/1.0', 'dep2/1.0'", client.out
        )
        self.assertIn(
            "Try to build locally from sources using the '--build=dep1/1.0 --build=dep2/1.0'",
            client.out,
        )
