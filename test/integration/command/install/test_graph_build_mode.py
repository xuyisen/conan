    ...
    client.save(
        {
            ...
            .with_require("foo/1.0@user/testing")
...
def check_if_build_from_sources(refs_modes, output):
    for ref, mode in refs_modes.items():
        if mode == "Build":
            ...
            assert (
                "{}/1.0@user/testing: Forced build from source".format(ref)
                not in output
            )
...
def test_install_build_single(build_all):
    ...
    build_all.assert_listed_binary(
        {
            "bar/1.0@user/testing": (bar_id, "Cache"),
...
def test_install_build_double(build_all):
    ...
    build_all.run(
        "install --requires=foobar/1.0@user/testing --build=foo/* --build=bar/*"
    )
    build_all.assert_listed_binary(
        {
            "bar/1.0@user/testing": (bar_id, "Build"),
            "foo/1.0@user/testing": (foo_id, "Build"),
            "foobar/1.0@user/testing": (foobar_id, "Cache"),
        }
    )
...
@pytest.mark.parametrize(
    "build_arg,mode", [("--build=", "Cache"), ("--build=*", "Build")]
)
def test_install_build_only(build_arg, mode, build_all):
    ...
    build_all.assert_listed_binary(
        {
            "bar/1.0@user/testing": (bar_id, mode),
...
@pytest.mark.parametrize(
    "build_arg,bar,foo,foobar",
    [("--build=", "Cache", "Build", "Cache"), ("--build=*", "Build", "Build", "Build")],
)
def test_install_build_all_with_single(build_arg, bar, foo, foobar, build_all):
    ...
    """
    build_all.run(
        "install --requires=foobar/1.0@user/testing --build=foo/* {}".format(build_arg)
    )
    build_all.assert_listed_binary(
        {
            "bar/1.0@user/testing": (bar_id, bar),
            "foo/1.0@user/testing": (foo_id, foo),
            "foobar/1.0@user/testing": (foobar_id, foobar),
        }
    )
    check_if_build_from_sources(
        {"foo": foo, "bar": bar, "foobar": foobar}, build_all.out
    )


@pytest.mark.parametrize(
    "build_arg,bar,foo,foobar",
    [("--build=", "Cache", "Cache", "Cache"), ("--build=*", "Build", "Cache", "Build")],
)
...
@pytest.mark.parametrize(
    "build_arg,bar,foo,foobar",
    [("--build=", "Cache", "Cache", "Cache"), ("--build=*", "Cache", "Cache", "Build")],
)
