...
def test_basic(relative_path):
    ...
    files = {"conanfile.py": GenConanfile().with_exports("*"), "main.cpp": "mymain"}
    client.save(files, path=source_folder)
    if relative_path:
        with client.chdir("current"):
            client.run(
                "export ../source --name=hello --version=0.1 --user=lasote --channel=stable"
            )
    else:
        client.run(
            "export source --name=hello --version=0.1 --user=lasote --channel=stable"
    ...
    expected_sums = {
        "conanfile.py": "7fbb7e71f5b665780ff8c407fe0b1f9e",
...
def test_path(relative_path):
    ...
    conanfile = textwrap.dedent("""
        ...
        class Pkg(ConanFile):
            ...
            """)
    if relative_path:
        client.save({"conan/conanfile.py": conanfile, "source/main.cpp": "mymain"})
        with client.chdir("current"):
            client.run(
                "export ../conan --name=hello --version=0.1 --user=lasote --channel=stable"
            )
    else:
        client.save({"current/conanfile.py": conanfile, "source/main.cpp": "mymain"})
        with client.chdir("current"):
            client.run(
                "export . --name=hello --version=0.1 --user=lasote --channel=stable"
            )
    ref_layoyt = client.exported_layout()
    reg_path = ref_layoyt.export()
    manif = FileTreeManifest.load(reg_path)
    ...
    expected_sums = {
        "conanfile.py": "6cdb33126a0408bffc0ad0ada66cb061",
