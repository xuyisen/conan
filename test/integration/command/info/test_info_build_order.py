...
def test_info_build_order():
        ...
        [
            {
                ...
                "depends": ["dep/0.1#4d670581ccb765839f2239cc8dff8fbd"],
                "packages": [
                    [
                        {
    ...
    c.run(
        "graph build-order consumer --build=missing --order-by=recipe --reduce --format=json"
    )
    bo_json = json.loads(c.stdout)
    assert bo_json["order_by"] == "recipe"
    assert bo_json["order"] == result
    ...
    assert "<body>" in c.stdout
    c.run(
        "graph build-order consumer --order-by=configuration --build=missing --format=html"
    )
    assert "<body>" in c.stdout
...
def test_info_build_order_configuration():
    ...
    c.run("export pkg --name=pkg --version=0.1")
    c.run(
        "graph build-order consumer --build=missing --order=configuration --format=json"
    )
    bo_json = json.loads(c.stdout)
    ...
    c.run(
        "graph build-order consumer --build=missing --order=configuration --reduce --format=json"
    )
    bo_json = json.loads(c.stdout)
    assert bo_json["order"] == result
...
def test_info_build_order_configuration_text_formatter():
    ...
    c.run("export pkg --name=pkg --version=0.1")
    c.run(
        "graph build-order consumer --build=missing --order=configuration --format=text"
    )
    assert (
        textwrap.dedent("""\
...
def test_info_build_order_options():
    ...
    c.save(
        {
            "tool/conanfile.py": GenConanfile().with_option("myopt", [1, 2, 3]),
            "dep1/conanfile.py": GenConanfile().with_tool_requirement(
                "tool/0.1", options={"myopt": 1}
            ),
            "dep2/conanfile.py": GenConanfile().with_tool_requirement(
                "tool/0.1", options={"myopt": 2}
            ),
            "consumer/conanfile.txt": "[requires]\ndep1/0.1\ndep2/0.1",
        }
    )
...
def test_info_build_order_merge_multi_product():
    ...
    c.run(
        "graph build-order consumer1  --build=missing --format=json",
        redirect_stdout="bo1.json",
    )
    c.run(
        "graph build-order consumer2  --build=missing --format=json",
        redirect_stdout="bo2.json",
    )
    c.run(
        "graph build-order-merge --file=bo1.json --file=bo2.json --format=json",
...
def test_info_build_order_merge_multi_product_configurations():
    ...
    c.run("export pkg --name=pkg --version=0.2")
    c.run(
        "graph build-order consumer1  --build=missing --order=configuration --format=json",
        redirect_stdout="bo1.json",
    )
    c.run(
        "graph build-order consumer2  --build=missing --order=configuration --format=json",
        redirect_stdout="bo2.json",
    )
    c.run(
        "graph build-order-merge --file=bo1.json --file=bo2.json --format=json",
...
def test_info_build_order_merge_conditionals():
    ...
    c.run("export pkg --name=pkg --version=0.1")
    c.run(
        "graph build-order consumer --format=json --build=missing -s os=Windows",
        redirect_stdout="bo_win.json",
    )
    c.run(
        "graph build-order consumer --format=json --build=missing -s os=Linux",
        redirect_stdout="bo_nix.json",
    )
    c.run(
        "graph build-order-merge --file=bo_win.json --file=bo_nix.json --format=json",
        redirect_stdout="bo3.json",
    )
...
def test_build_order_missing_package_check_error():
    ...
    exit_code = c.run(
        "graph build-order consumer --build='pkg/*' --order=configuration --format=json",
        assert_error=True,
    )
    bo_json = json.loads(c.stdout)
    assert bo_json["order_by"] == "configuration"
...
class TestBuildOrderReduce:
    ...
    def test_build_order_reduce(self, order):
        ...
        c.run("remove libc:* -c")
        c.run(
            f"graph build-order consumer --order={order} --build=missing --reduce --format=json"
        )
        bo_json = json.loads(c.stdout)
    ...
    def test_build_order_merge_reduce(self, order):
        ...
        c.save(
            {
                "liba/conanfile.py": GenConanfile("liba", "0.1").with_settings("os"),
                "libb/conanfile.py": GenConanfile("libb", "0.1")
                .with_settings("os")
                .with_requires("liba/0.1"),
                "libc/conanfile.py": GenConanfile("libc", "0.1")
                .with_settings("os")
        ...
        c.run("remove libc:* -c")
        c.run(
            f"graph build-order consumer --order={order} --build=missing -s os=Windows "
            "--format=json",
            redirect_stdout="windows.json",
        )
        c.run(
            f"graph build-order consumer --order={order} --build=missing -s os=Linux "
            "--format=json",
            redirect_stdout="linux.json",
        )

        c.run(
            "graph build-order-merge --file=windows.json --file=linux.json --reduce "
            "--format=json"
        )
        bo_json = json.loads(c.stdout)
        order_json = bo_json["order"]
        ...
        if order == "recipe":
            ...
            assert len(level0) == 2
            liba1 = (
                "liba/0.1#8c6ed89c12ab2ce78b239224bd7cb79e:"
    ...
    def test_error_reduced(self):
        ...
        c.run(
            "graph build-order . --order-by=recipe --reduce --format=json",
            redirect_stdout="bo2.json",
        )
        c.run(
            "graph build-order-merge --file=bo1.json --file=bo2.json", assert_error=True
        )
        assert "ERROR: Reduced build-order file cannot be merged: bo2.json"
        # different order
        c.run(
            "graph build-order-merge --file=bo2.json --file=bo1.json", assert_error=True
        )
        assert "ERROR: Reduced build-order file cannot be merged: bo2.json"
    ...
    def test_error_different_orders(self):
        ...
        c.run("graph build-order . --format=json", redirect_stdout="bo1.json")
        c.run(
            "graph build-order . --order-by=recipe --format=json",
            redirect_stdout="bo2.json",
        )
        c.run(
            "graph build-order . --order-by=configuration --format=json",
            redirect_stdout="bo3.json",
        )
        c.run("graph build-order-merge --file=bo1.json --file=bo2.json")
        # Not error
        c.run(
            "graph build-order-merge --file=bo1.json --file=bo3.json", assert_error=True
        )
        assert "ERROR: Cannot merge build-orders of recipe!=configuration" in c.out
        c.run(
            "graph build-order-merge --file=bo2.json --file=bo3.json", assert_error=True
        )
        assert "ERROR: Cannot merge build-orders of recipe!=configuration" in c.out
        # different order
        c.run(
            "graph build-order-merge --file=bo3.json --file=bo2.json", assert_error=True
        )
    ...
    def test_merge_missing_error(self):
        ...
        tc.run("export dep")
        tc.run(
            "graph build-order --order=recipe --requires=dep/1.0 --format=json",
            assert_error=True,
            redirect_stdout="order.json",
        )
        tc.run(
            "graph build-order-merge --file=order.json --file=order.json --format=json",
            assert_error=True,
        )
        assert (
            "dep/1.0:da39a3ee5e6b4b0d3255bfef95601890afd80709: Missing binary" in tc.out
        )
    ...
    def test_merge_invalid_error(self):
        ...
        assert (
            "dep/1.0:da39a3ee5e6b4b0d3255bfef95601890afd80709: Invalid configuration"
            in tc.out
        )
        assert "IndexError: list index out of range" not in tc.out
    ...
    def test_reduce_should_remove_recipe(self):
        ...
        tc.run(
            "graph build-order -s os=Windows --build=missing --order=recipe --requires=dep/1.0 "
            "--format=json",
            redirect_stdout="windows.json",
        )
        tc.run(
            "graph build-order -s os=Linux --build=missing --order=recipe --requires=dep/1.0 "
            "--format=json",
            redirect_stdout="linux.json",
        )
        tc.run(
...
def test_multi_configuration_profile_args():
    ...
    c.run(
        f"graph build-order consumer --format=json --build=missing -s os=Windows {args} "
        "--order-by=recipe",
        redirect_stdout="bo_win.json",
    )
    c.run(
        f"graph build-order consumer --format=json --build=missing -s os=Linux {args} "
        "--order-by=recipe",
        redirect_stdout="bo_nix.json",
    )
    c.run(
...
def test_build_order_space_in_options():
    ...
    tc.run("create dep")
    tc.run(
        "graph build-order . --order-by=configuration --build=dep/1.0 -f=json",
        redirect_stdout="order.json",
    )
    order = json.loads(tc.load("order.json"))
    assert (
        order["order"][0][0]["build_args"]
...
def test_build_order_build_context_compatible():
    ...
    foo = textwrap.dedent("""
        ...
        """)
    bar = (
        GenConanfile("bar", "1.0")
        .with_settings("os", "compiler")
        .with_tool_requirement("foo/1.0")
    )
    ...
    profile = textwrap.dedent("""
        ...
        os=Linux
        """)
    c.save({"conanfile_foo.py": foo, "conanfile_bar.py": bar, "profile": profile})
    ...
    for approach in (
        "--build=missing -s foo/*:compiler.cppstd=17",
        '--build="compatible:foo/*" --build="missing:bar/*"',
        '--build="missing:foo/*" --build="compatible:foo/*" --build="missing:bar/*"',
    ):
        c.run(
            f"graph build-order --require=foo/1.0 --require=bar/1.0 -pr:a profile {approach}"
        )
        c.assert_listed_binary(
            {
                "foo/1.0": ["4e2ae338231ae18d0d43b9e119404d2b2c416758", "Build"],
...
def test_info_build_order_editable():
    ...
    c.run(
        "graph build-order consumer --build=missing --build=editable -f=json --order-by=recipe"
    )
    bo_json = json.loads(c.stdout)
    ...
    c.run(
        "graph build-order consumer --build=missing --build=editable -f=json "
