...
class TestNewCommandUserTemplate:
    ...
    def test_user_template(self, folder):
        ...
        save(
            os.path.join(
                client.cache_folder, f"templates/command/new/{folder}/conanfile.py"
            ),
            template1,
        )
    ...
    def test_user_template_filenames(self):
        ...
        save(
            os.path.join(
                client.cache_folder, "templates/command/new/mytemplate/{{name}}"
            ),
            "Hi!",
        )
        client.run("new mytemplate -d name=pkg.txt")
...
class TestNewErrors:
    ...
    def test_duplicated(self):
        ...
        client.run(
            "new cmake_lib -d name=pkg -d version=0.1 -d version=0.2", assert_error=True
        )
    ...
    def test_new_change_folder(self):
        ...
        assert os.path.exists(
            os.path.join(client.current_folder, "myfolder", "conanfile.py")
        )
