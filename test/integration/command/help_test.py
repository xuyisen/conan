        ...
        for command, info in commands.items():
            if len(info._subcommands) > 0:
                for subcommand in info._subcommands.values():
                    with patch(
                        "conan.cli.command.ConanArgumentParser.parse_args",
                        side_effect=Exception("called"),
                    ) as mock_run:
                        try:
                            ...
                            assert mock_run.called, (
                                f'Command "conan {command} {subcommand.name}" did not call parse_args()'
                            )
            else:
                with patch(
                    "conan.cli.command.ConanArgumentParser.parse_args",
                    ...
                    try:
                        ...
                        assert mock_run.called, (
                            f'Command "conan {command}" did not call parse_args()'
                        )
