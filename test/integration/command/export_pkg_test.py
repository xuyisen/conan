...
class TestExportPkg:
        ...
        client.save(
            {
                ...
                "pkgb/conanfile.py": GenConanfile("pkgb", "0.1").with_requires(
                    "pkgc/0.1"
                ),
                "pkga/conanfile.py": GenConanfile("pkga", "0.1").with_requires(
                    "pkgb/0.1"
                ),
            }
        )
    ...
    def test_package_folder_errors(self):
        ...
        client.run(
            "export-pkg . --name=hello --version=0.1 --user=lasote --channel=stable"
        )
        assert (
            "conanfile.py (hello/0.1@lasote/stable): package(): "
    ...
    def test_profile_environment(self):
        ...
        profile = dedent("""
            ...
            """)
        client = TestClient()
        client.save({CONANFILE: conanfile, "myprofile": profile})
        client.run(
            "export-pkg . --name=hello --version=0.1 --user=lasote --channel=stable "
            " -pr=myprofile"
        )
        assert (
            "conanfile.py (hello/0.1@lasote/stable): ENV-VALUE: MYCUSTOMVALUE!!!"
            in client.out
        )
    ...
    def test_partial_references(self):
        ...
        assert (
            "conanfile.py (hello/0.1@conan/stable): package(): "
            "Packaged 1 '.txt' file: file.txt"
        ) in client.out
        ...
        client.save({CONANFILE: conanfile, "file.txt": "txt contents"})
        client.run(
            "export-pkg . --name=anyname --version=1.222 --user=conan --channel=stable"
        )
        assert (
            "conanfile.py (anyname/1.222@conan/stable): package(): "
            "Packaged 1 '.txt' file: file.txt"
        ) in client.out
    ...
    def package(self):
        ...
        settings = (
            "-s os=Windows -s compiler=gcc -s compiler.version=4.9 "
            "-s compiler.libcxx=libstdc++ -s build_type=Release -s arch=x86"
        )
        client.run(
            "export-pkg . --name=hello1 --version=0.1 --user=lasote --channel=stable %s"
            % settings
        )

        consumer = (
            GenConanfile()
            .with_requires("hello1/0.1@lasote/stable")
    ...
    def test_export_pkg_json(self):
        ...
        revs = pkglist["Local Cache"]["pkg/0.1"]["revisions"][
            ...
            class TestConan(ConanFile):
                ...
                def package(self):
                    copy(self, "*.h", os.path.join(self.source_folder, "src"),
                         os.path.join(self.package_folder, "include"))
            """)
        client.save({CONANFILE: conanfile, "src/header.h": "contents"})
...
def test_build_policy_never():
    ...
    assert (
        "conanfile.py (pkg/1.0): package(): Packaged 1 '.h' file: header.h"
        in client.out
    )
...
def test_build_policy_never_missing():
    ...
    client.save(
        {
            "conanfile.py": GenConanfile().with_class_attribute(
                'build_policy = "never"'
            ),
...
def test_export_pkg_dont_update_src():
    ...
    c.save({"conanfile.py": conanfile, "src/hello.cpp": "old code!"})
    c.run("install .")
    c.run("build .")
    c.run("export-pkg .")
...
def test_negate_tool_requires():
    ...
    profile = textwrap.dedent("""
        ...
        !mypkg/*:cmake/3.24
        """)
    c.save({"myprofile": profile, "conanfile.py": GenConanfile("mypkg", "0.1")})
...
def test_export_pkg_test_package_build_require():
    ...
    c.save(
        {
            "conanfile.py": GenConanfile("pkg", "1.0").with_setting("os"),
            "test_package/conanfile.py": test_conanfile,
        }
    )

    c.run(
        "export-pkg . -s:b os=Windows -s:h os=Linux --build-require --lockfile-out=conan.lock"
    )
    assert "test_package" in c.out
    assert "RUN TEST PACKAGE!!!!" in c.out
    ...
    c.save(
        {
            ...
            "pkg/conanfile.py": GenConanfile("pkg", "1.0").with_python_requires(
                "tool/1.0"
            ),
        }
    )
...
def test_remote_none_tool_requires():
    ...
    c.save(
        {
            "tool/conanfile.py": GenConanfile("tool", "0.1").with_settings("compiler"),
    ...
    settings = (
        "-s:b compiler=gcc -s:b compiler.version=9 -s:b compiler.libcxx=libstdc++11"
    )
...
def test_export_pkg_set_version_python_requires():
    ...
    pkg = textwrap.dedent("""
        ...
        """)
    c.save({"pyreq/conanfile.py": py_require, "pkg/conanfile.py": pkg})
