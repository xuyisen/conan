...
class TestRemovePackageRevisions:
    ...
    def test_remove_package_revision(self):
        ...
        ref = RecipeReference.loads(
            "foobar/0.1@user/testing#4d670581ccb765839f2239cc8dff8fbd"
        )
        pref_arg = PkgReference(ref, NO_SETTINGS_PACKAGE_ID)
        pref = PkgReference(
            ref, NO_SETTINGS_PACKAGE_ID, "0ba8627bd47edc3a501e8f0eb9a79e5e"
        )
...
def _populated_client_base():
    ...
    package_lines = (
        'save(self, os.path.join(self.package_folder, "foo.txt"), '
...
def populated_client(_populated_client_base):
    ...
    shutil.copytree(
        _populated_client_base.servers["default"].test_server._base_path,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "remove": "*/*#z*",
            "recipes": ["foo/1.0@user/channel", "foo/1.0", "bar/1.1", "fbar/1.1"],
        },
        ...
        {
            "remove": "*/*#*:*",
            "recipes": ["bar/1.1", "foo/1.0@user/channel", "foo/1.0", "fbar/1.1"],
        },
        {
            "remove": "foo/1.0@user/channel:*",
            "recipes": ["bar/1.1", "foo/1.0@user/channel", "foo/1.0", "fbar/1.1"],
        },
        {
            "remove": "foo/*@",
            "recipes": ["foo/1.0@user/channel", "bar/1.1", "fbar/1.1"],
        },
        {"remove": "foo", "recipes": ["bar/1.1", "fbar/1.1"]},
        {"remove": "*#", "recipes": []},
        {"remove": "*/*#", "recipes": []},
...
def test_new_remove_recipe_revisions_expressions(populated_client, with_remote, data):
    ...
    populated_client.run(
        "remove {} -c {}".format(data["remove"], r), assert_error=error
    )
    if not error:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "remove": "bar/1.1#*:*{}".format(pkg_id_release[-12:]),
            "prefs": [bar_rrev2_debug],
        },
        {
            "remove": "{}:*{}*".format(bar_rrev2, pkg_id_debug[5:15]),
            "prefs": [bar_rrev2_release],
        },
        {
            "remove": "*/*#*:*{}*".format(pkg_id_debug[5:15]),
            "prefs": [bar_rrev2_release],
        },
        {
            "remove": '*/*#*:* -p build_type="fake"',
            "prefs": [bar_rrev2_release, bar_rrev2_debug],
        },
        ...
        {"remove": "bar/1.1:234234*", "prefs": [bar_rrev2_debug, bar_rrev2_release]},
        {
            "remove": "bar/1.1:* -p os=Windows",
            "prefs": [bar_rrev2_debug, bar_rrev2_release],
        },
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        {
            "remove": "{}#*kk*".format(bar_rrev2_release),
            "prevs": [bar_rrev2_release_prev1, bar_rrev2_release_prev2],
        },
        {"remove": "{}#*".format(bar_rrev2_release), "prevs": []},
        {
            "remove": '{}*#{}* -p "build_type=Debug"'.format(
                bar_rrev2_release, bar_prev2[:3]
            ),
            "prevs": [bar_rrev2_release_prev1, bar_rrev2_release_prev2],
        },
        {
            "remove": '{}*#{}* -p "build_type=Release"'.format(
                bar_rrev2_release, bar_prev2[:3]
            ),
            "prevs": [bar_rrev2_release_prev1],
        },
        {
            "remove": '{}*#* -p "build_type=Release"'.format(bar_rrev2_release),
            "prevs": [],
        },
        {
        ...
        {"remove": "{}#".format(bar_rrev2_release), "prevs": []},
        {
            "remove": "{}#latest".format(bar_rrev2_release),
            "prevs": [bar_rrev2_release_prev1],
        },
        {
            "remove": "{}#!latest".format(bar_rrev2_release),
            "prevs": [bar_rrev2_release_prev2],
        },
        {
            "remove": "{}#~latest".format(bar_rrev2_release),
            "prevs": [bar_rrev2_release_prev2],
...
def test_package_query_no_package_ref(populated_client):
    ...
    assert (
        "--package-query supplied but the pattern does not match packages"
        in populated_client.out
    )
...
def _get_revisions_recipes(client, ref, with_remote):
    ...
    with client.mocked_servers():
        ...
        try:
            return set(
                [r.repr_notime() for r in api.list.recipe_revisions(ref, remote=remote)]
            )
        except NotFoundException:
            return set()
