...
def _drop_lru_column(db_folder):
    ...
    connection = sqlite3.connect(
        db, isolation_level=None, timeout=1, check_same_thread=False
    )
    ...
    try:
        for table in ("recipes", "packages"):
            ...
            connection.execute(
                f"CREATE TABLE {table}_backup AS SELECT {columns} FROM {table};"
            )
...
@pytest.mark.parametrize(
    ["plugin_path", "string_replace", "new_string"],
    [
...
def test_migration_profile_checker_plugin(plugin_path, string_replace, new_string):
    ...
    profile_plugin_path = os.path.join(
        t.cache_folder, "extensions", "plugins", plugin_path
    )
    ...
    profile_plugin_path2 = os.path.join(
        t2.cache_folder, "extensions", "plugins", plugin_path
    )
    contents = load(profile_plugin_path2)
    contents = contents.replace(string_replace, new_string)
    contents = contents.replace(
        "This file was generated by Conan",
        "This file is from ACME corp, please don't touch it.",
    )
...
def test_back_migrations():
    ...
    for number in (1, 2, 3):
        migration_file = os.path.join(
            t.cache_folder, "migrations", f"2.100.0_{number}-migrate.py"
        )
        ...
        wrong_migration_file = os.path.join(
            t.cache_folder, "migrations", f"2.0_{number}-migrate.py"
        )
    ...
    for number in (1, 2, 3):
        assert (
            f"WARN: Applying downgrade migration 2.100.0_{number}-migrate.py" in t.out
        )
...
def test_back_default_compatibility_migration():
    ...
    with patch(
        "conan.api.conan_api.ClientMigrator", new=lambda *args, **kwargs: migrator
    ):
        t.run("-v")  # Fire the backward migration
