...
def test_layout_generate():
    ...
    assert os.path.exists(
        os.path.join(c.current_folder, "mybuild", "bin", "myfile.txt")
    )
    assert os.path.exists(
        os.path.join(c.current_folder, "mybuild", "lib", "myfile.txt")
    )
    assert os.path.exists(
        os.path.join(c.current_folder, "mybuild", "include", "myfile.txt")
    )
...
def test_generate_build_folder_test_package():
    ...
    c.run("create .")
    assert "build_folder in test_package: True" in c.out
...
class TestCustomTestPackage:
    def test_custom_test_package(self):
        ...
        conanfile = GenConanfile("pkg", "0.1").with_class_attribute(
            'test_package_folder="mytest"'
        )
        c.save(
            {
                "conanfile.py": conanfile,
                "mytest/conanfile.py": GenConanfile().with_test(
                    "self.output.info('MYTEST!')"
                ),
                "mytest2/conanfile.py": GenConanfile().with_test(
                    "self.output.info('MYTEST2!')"
                ),
            }
        )
        c.run("create .")
        ...
        c.save(
            {
                ...
                "pkg/my/test/conanfile.py": GenConanfile().with_test(
                    "self.output.info('MYTEST!')"
                ),
            }
        )
    ...
    def test_custom_test_package_sibling(self):
        ...
        c.save(
            {
                "pkg/conan/conanfile.py": conanfile,
                "pkg/my/test/conanfile.py": GenConanfile().with_test(
                    "self.output.info('MYTEST!')"
                ),
            }
    ...
    def test_custom_test_package_export_pkg(self):
        ...
        c.save(
            {
                "pkg/conanfile.py": conanfile,
                "pkg/mytest/conanfile.py": GenConanfile().with_test(
                    "self.output.info('MYTEST!')"
                ),
            }
        )
