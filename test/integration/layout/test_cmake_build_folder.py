...
def test_cmake_layout_build_folder(cmd):
    ...
    assert os.path.exists(
        os.path.join(abs_build, "Release", "generators", "conan_toolchain.cmake")
    )
    ...
    assert os.path.exists(
        os.path.join(new_folder, "Release", "generators", "conan_toolchain.cmake")
    )
    assert not os.path.exists(os.path.join(c.current_folder, "build"))
...
def test_test_cmake_layout_build_folder_test_package():
    ...
    c.run(
        f'create . -c tools.cmake.cmake_layout:test_folder="{abs_build}" '
    ...
    c.run("create . -c tools.cmake.cmake_layout:build_folder_vars=[]")
...
def test_test_cmake_layout_build_folder_test_package_temp():
    ...
    assert os.path.exists(
        os.path.join(build_folder, "generators", "conan_toolchain.cmake")
    )
    assert not os.path.exists(os.path.join(c.current_folder, "test_package", "build"))

    c.run("test test_package pkg/0.1 -pr=profile")
    build_folder = re.search(r"Test package build folder: (\S+)", str(c.out)).group(1)
    assert os.path.exists(
        os.path.join(build_folder, "generators", "conan_toolchain.cmake")
    )
...
def test_cmake_layout_build_folder_editable():
    ...
    c.save(
        {
            "pkga/conanfile.py": conanfile.format(name="pkga"),
            "pkgb/conanfile.py": conanfile.format(name="pkgb"),
            "app/conanfile.py": GenConanfile()
            .with_requires("pkga/0.1", "pkgb/0.1")
    ...
    conf = (
        '-c tools.cmake.cmake_layout:build_folder="../../mybuild" '
...
def test_cmake_layout_editable_output_folder():
    ...
    c.run('editable add pkga --output-folder="../mybuild"')
    c.run('editable add pkgb --output-folder="../mybuild"')
    conf = "-c tools.cmake.cmake_layout:build_folder_vars=\"['self.name']\""
    c.run(f"install app {conf} --build=editable")
    ...
    data = c.load("app/pkgb-release-data.cmake")
    assert (
        'pkgb_LIB_DIRS_RELEASE "${pkgb_PACKAGE_FOLDER_RELEASE}/build/pkgb/Release'
        in data
    )
