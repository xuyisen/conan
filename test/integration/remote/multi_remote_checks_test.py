...
class Pkg(ConanFile):
    pass"""
        ...
        self.assertIn(
            "pkg/0.1@lasote/testing: Retrieving package "
            "%s from remote 'server1'" % NO_SETTINGS_PACKAGE_ID,
            client.out,
        )
        ...
        self.assertIn(
            "pkg/0.1@lasote/testing: Retrieving package "
            "%s from remote 'server2'" % NO_SETTINGS_PACKAGE_ID,
            client.out,
        )
...
"""
        client.save({"conanfile.py": conanfile})
        client.run(
            "create . --name=pkg --version=0.1 --user=lasote --channel=testing -o pkg/*:opt=1"
        )
        client.run("upload pkg* -r=server1 --confirm")
        client.run("remove *:* -c")
        client.run(
            "create . --name=pkg --version=0.1 --user=lasote --channel=testing -o pkg/*:opt=2"
        )
        ...
        client.run(
            "install --requires=pkg/0.1@lasote/testing -o pkg/*:opt=2 -r=server2"
        )
        client.assert_listed_binary(
            {"pkg/0.1@lasote/testing": (package_id2, "Download (server2)")}
        )
        self.assertIn(
            f"pkg/0.1@lasote/testing: Retrieving package {package_id2} "
            "from remote 'server2'",
            client.out,
        )
        ...
        # Nothing to update
        client.run(
            "install --requires=pkg/0.1@lasote/testing -o pkg/*:opt=2 -r=server2 -u"
        )
        client.assert_listed_binary({"pkg/0.1@lasote/testing": (package_id2, "Cache")})

        # Build missing
        client.run(
            "install --requires=pkg/0.1@lasote/testing -o pkg/*:opt=3 -r=server2",
            assert_error=True,
        )
        self.assertIn(
            "ERROR: Missing prebuilt package for 'pkg/0.1@lasote/testing'", client.out
        )

        client.run(
            "install --requires=pkg/0.1@lasote/testing -o pkg/*:opt=3",
            assert_error=True,
        )
        self.assertIn(
            "ERROR: Missing prebuilt package for 'pkg/0.1@lasote/testing'", client.out
...
def test_version_range_multi_remote():
    ...
    # First found
    assert (
        "pkg1/1.1#4d670581ccb765839f2239cc8dff8fbd - Downloaded (server1)" in client.out
    )
    assert (
        "pkg2/1.3#4d670581ccb765839f2239cc8dff8fbd - Downloaded (server1)" in client.out
    )
    ...
    assert (
        "pkg1/1.3#4d670581ccb765839f2239cc8dff8fbd - Downloaded (server3)" in client.out
    )
    assert (
        "pkg2/1.3#4d670581ccb765839f2239cc8dff8fbd - Downloaded (server1)" in client.out
    )
