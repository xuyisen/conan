...
def test_user_overrides():
    ...
    c.run(
        "graph info game --lockfile=conan.lock --lockfile-out=new.lock --lockfile-partial"
    )
    ...
    c.run(
        "graph info game --lockfile=conan.lock --lockfile-partial --lockfile-out=new.lock"
    )
...
def test_user_build_overrides():
    ...
    c.save(
        {
            ...
            "engine/conanfile.py": GenConanfile("engine", "1.0").with_build_requires(
                "cmake/[*]"
            ),
        }
    )
    ...
    c.run(
        "graph info engine --lockfile=conan.lock --lockfile-out=new.lock --lockfile-partial"
    )
    ...
    c.save(
        {
            ...
            "engine/conanfile.py": GenConanfile("engine", "1.0").with_python_requires(
                "pytool/[*]"
            ),
        }
    )
...
def test_lock_add_error():
    ...
    c = TestClient(light=True)
    c.run("lock add --requires=math/1.0:pid1", assert_error=True)
    assert (
        "ERROR: Invalid recipe reference 'math/1.0:pid1' is a package reference"
        in c.out
    )
...
class TestLockRemove:
    ...
    def test_lock_remove_revisions(self, args, removed):
        ...
        for pkg in {
            "math/1.0#123456789abcdef",
            "math/1.0#85d927a4a067a531b1a9c7619522c015",
            "engine/1.0#fd2b006646a54397c16a1478ac4111ac",
        }.difference(removed):
            assert pkg in lock
...
class TestLockUpgrade:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                "cmake/1.0",
                "cmake/1.1",
            ),  # TODO there is not a --build-requires
    ...
    def test_lock_upgrade_path(self):
        ...
        c.run("export libb --version=1.0")
        c.run("export libc --version=1.0")
        c.run("export libd --version=1.0")
        c.save(
            {
                "conanfile.py": GenConanfile()
                .with_requires("liba/[>=1.0 <2]")
        ...
        c.run("export liba --version=1.9")
        c.run("export libb --version=1.1")
        c.run("export libb --version=1.2")
        c.run("export libc --version=1.1")
        c.run("export libd --version=1.1")
        c.run(
            "lock upgrade . --update-requires=liba/1.0 --update-requires=libb/[*] --update-build-requires=libc/[*] --update-python-requires=libd/1.0"
        )
        lock = c.load("conan.lock")
    ...
    def test_lock_upgrade_new_requirement(self):
        ...
        c.save(
            {
                "liba/conanfile.py": GenConanfile("liba").with_requires("libb/1.0"),
        ...
        c.run("export libc --version=1.0")
        c.run("export libd --version=1.0")
        c.save({"conanfile.py": GenConanfile().with_requires("liba/[>=1.0 <2]")})
        c.run("lock create .")

        c.save({"libb/conanfile.py": GenConanfile("libb").with_requires("libd/1.0")})
        c.run("export libb --version=2.0")
        ...
        assert (
            "libc/1.0" in lock
        )  # TODO: libc should be removed from lockfile? It is not required anymore...
