...
def test_lock_pyrequires_prereleases():
    ...
    tc.save(
        {
            "dep/conanfile.py": GenConanfile("dep", "1.0-0.1"),
            "app/conanfile.py": GenConanfile("app", "1.0").with_python_requires(
                "dep/[>=0]"
            ),
        }
    )
...
def test_lock_pyrequires_create():
    ...
    c.save(
        {
            "conanfile.py": GenConanfile("tool", "0.1").with_package_type(
                "python-require"
            )
        }
    )
