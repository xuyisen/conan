...
def test_graph_build_order_override_error():
    ...
    c.save(
        {
            "zlib/conanfile.py": GenConanfile("zlib"),
            ...
            .with_requirement("zlib/3.0", override=True),
        }
    )
    ...
    c.run(
        "graph build-order --requires=libc/0.1 --lockfile=output.lock --order-by=configuration "
...
def test_single_config_decentralized_overrides_nested():
    ...
    c.save(
        {
            "libf/conanfile.py": GenConanfile("libf"),
            ...
            .with_requirement("libd/1.0")
            .with_requirement("libf/2.0", override=True),
            "libb/conanfile.py": GenConanfile("libb", "1.0").with_requires("libc/1.0"),
            "toola/conanfile.py": GenConanfile("toola", "1.0")
            .with_requirement("libb/1.0")
            .with_requirement("libf/3.0", override=True),
            "pkga/conanfile.py": GenConanfile("pkga", "1.0").with_tool_requires(
                "toola/1.0"
            ),
        }
    ...
    assert lock["overrides"] == {"libf/1.0": ["libf/3.0"], "libf/2.0": ["libf/3.0"]}

    c.run(
        "graph build-order pkga --lockfile=pkga/conan.lock --format=json --build=missing"
    )
    to_build = json.loads(c.stdout)
    for level in to_build:
        for elem in level:
    ...
    c.save(
        {
            ...
            "libc/conanfile.py": GenConanfile("libc", "1.0")
            .with_requirement("libd/1.0")
            .with_requirement("libf/2.0", override=override, force=force),
            "libb/conanfile.py": GenConanfile("libb", "1.0").with_requires("libc/1.0"),
            "toola/conanfile.py": GenConanfile("toola", "1.0")
            .with_requirement("libb/1.0")
            .with_requirement("libf/3.0", override=override, force=force),
            "toola1/conanfile.py": GenConanfile("toola", "1.1")
            .with_requirement("libb/1.0")
            .with_requirement("libf/4.0", override=override, force=force),
            "toola2/conanfile.py": GenConanfile("toola", "1.2").with_requirement(
                "libb/1.0"
            ),
            "pkga/conanfile.py": GenConanfile("pkga", "1.0").with_tool_requires(
                "toola/1.0"
...
@pytest.mark.parametrize("forced", [False, True])
def test_single_config_decentralized_overrides_multi_replace_requires(
    replace_pattern, forced
):
    r"""same scenario as "test_single_config_centralized()", but distributing the build in
    different build servers, using the "build-order"
    Now with overrides
    ...
    c.run(
        "graph build-order pkgc --lockfile=pkgc/conan.lock --format=json -pr:b=profile "
        "--build=missing"
    )
