...
def test_conanfile_txt_strict(requires):
    ...
    assert (
        f"Requirement 'pkg/[>1.0]@user/testing' not in lockfile '{kind}'" in client.out
    )
    ...
    client.run(
        "install consumer/conanfile.txt --lockfile-partial --lockfile-out=conan.lock"
    )
    assert "pkg/1.2@user/testing" in client.out
    ...
    client.run(
        "lock create consumer/conanfile.txt --lockfile-out=conan.lock --lockfile-clean"
    )
    lock = client.load("conan.lock")
    assert "pkg/1.2" in lock
...
def test_conditional_os(requires):
    ...
    client.run(
        "lock create consumer/conanfile.txt  --lockfile-out=consumer.lock -s os=Windows"
        " -s:b os=Windows"
    )
    assert "win/0.1#" in client.out
    assert "pkg/0.1#" in client.out
    client.run(
        "lock create consumer/conanfile.txt  --lockfile=consumer.lock "
    ...
    client.run(
        "install consumer --lockfile=consumer.lock -s os=Windows -s:b os=Windows"
    )
    assert "win/0.1#" in client.out
...
def test_conditional_same_package(requires):
    ...
    client.run(
        "lock create consumer/conanfile.txt  --lockfile-out=conan.lock -s os=Windows"
        " -s:b os=Windows"
    )
    assert "dep/0.1#" in client.out
    assert "dep/0.2" not in client.out
    client.run(
        "lock create consumer/conanfile.txt  --lockfile=conan.lock "
    ...
    client.run(
        "install consumer --lockfile=conan.lock --lockfile-out=win.lock -s os=Windows"
        " -s:b os=Windows"
    )
    assert "dep/0.1#" in client.out
    assert "dep/0.2" not in client.out
    ...
    client.run(
        "install consumer --lockfile=conan.lock --lockfile-out=linux.lock -s os=Linux"
        " -s:b os=Linux"
    )
    assert "dep/0.2#" in client.out
    assert "dep/0.1" not in client.out
...
def test_conditional_compatible_range(requires):
    ...
    client.run(
        "lock create consumer/conanfile.txt  --lockfile-out=conan.lock -s os=Linux"
        " -s:b os=Linux"
    )
    assert "dep/0.2#" in client.out
    assert "dep/0.1" not in client.out
...
class TestLockTestPackage:
    ...
    def client(self):
        ...
        c.save(
            {
                "cmake/conanfile.py": GenConanfile("cmake"),
    ...
    def test_lock_tool_requires_test(self, client):
        ...
        with c.chdir("app"):
            ...
            c.run(
                "lock create test_package --lockfile=conan.lock --lockfile-out=conan.lock"
            )
            lock = c.load("conan.lock")
    ...
    def test_create_lock_tool_requires_test(self, client):
        ...
        with c.chdir("app"):
            ...
            assert "dep/1.0" in lock
            c.run(
                "test test_package app/1.0 --lockfile-partial --lockfile=conan.lock "
    ...
    def test_test_package_lockfile(self):
        ...
        c.save(
            {
                "pkga/conanfile.py": GenConanfile("pkga", "1.0"),
        ...
        c.run(
            "test pkgb/test_package pkgb/1.0 --lockfile=pkgb/conan.lock --lockfile-partial"
        )
        assert "Using lockfile:" in c.out

        # alternative 3, create the lockfile in the test_package
        c.run(
            "lock create pkgb/test_package --lockfile=pkgb/conan.lock "
            "--lockfile-out=pkgb/test_package/conan.lock"
        )
        lockfile = c.load("pkgb/test_package/conan.lock")
        assert "pkga/1.0" in lockfile
        c.run(
            "test pkgb/test_package pkgb/1.0 --lockfile=pkgb/test_package/conan.lock "
            "--lockfile-partial"
        )
...
class TestErrorDuplicates:
    def test_error_duplicates(self):
        ...
        pkg = textwrap.dedent("""
            ...
            class Pkg(ConanFile):
                ...
                """)
        c.save(
            {"dep/conanfile.py": GenConanfile("dep", "0.1"), "pkg/conanfile.py": pkg}
        )
    ...
    def test_error_duplicates_revisions(self):
        ...
        c.save(
            {
                "dep/conanfile.py": GenConanfile("dep", "0.1"),
                "dep2/conanfile.py": GenConanfile("dep", "0.1").with_class_attribute(
                    "potato=42"
                ),
...
def test_revision_timestamp():
    ...
    c.run("export pkg")
    c.save(
        {"pkg/conanfile.py": GenConanfile("pkg", "0.1").with_class_attribute("_my=1")}
    )
    # revision 1
    c.run("export pkg")
    ...
    c.save(
        {"pkg/conanfile.py": GenConanfile("pkg", "0.1").with_class_attribute("_my=2")}
    )
    # revision 2
    time.sleep(1)
    c.run("export pkg")
    ...
    server_timestamp = list_json["default"]["pkg/0.1"]["revisions"][latest_rrev][
        "timestamp"
    ]
...
def test_error_test_explicit():
    ...
    client = TestClient(light=True)
    test = (
        GenConanfile().with_test("pass").with_class_attribute("test_type = 'explicit'")
    )
    client.save(
        {"conanfile.py": GenConanfile("pkg", "0.1"), "test_package/conanfile.py": test}
    )
...
def test_lock_error_create():
    ...
    c = TestClient(light=True)
    c.save(
        {"conanfile.py": GenConanfile("pkg", "0.1").with_package_type("build-scripts")}
    )
...
def test_lock_error():
    ...
    c.save(
        {
            "recipes/transitive_dep/conanfile.py": transitive_dep,
    ...
    c.run(
        f"graph build-order recipes/consumer {settings} --build=missing --order-by=configuration "
        "--reduce  --format=json",
    ...
    c.run(
        f"install --tool-requires={ref} {settings} --build={ref} "
    ...
    c.run(
        f"install --requires={ref} {settings} --build={ref} "
        "--lockfile=recipes/consumer/conan.lock "
    )
