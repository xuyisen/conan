...
def client_setup():
    ...
    c.save_home(
        {"global.conf": "core.package_id:default_unknown_mode=recipe_revision_mode"}
    )
    pkb_requirements = """
...
def test_single_config_centralized(client_setup):
    ...
    c.run(
        "create pkgb --name=pkgb --version=0.1 -s os=Windows "
    ...
    c.save(
        {
            "pkgc/conanfile.py": conanfile.format(
                requires='requires="pkgb/0.1#d03e920d532beeeb198cd886095bcca1"'
            )
        }
    )
    c.run("create pkgc --name=pkgc --version=0.1 -s os=Windows")
...
def test_single_config_centralized_change_dep(client_setup):
    ...
    c.save(
        {
            "pkgb/conanfile.py": conanfile.format(requires='requires="pkgj/0.1"'),
    ...
    # Test that pkgb/0.1 new revision works
    c.run(
        "create pkgb --name=pkgb --version=0.1 -s os=Windows --lockfile-partial "
        "--lockfile=app1.lock --lockfile-out=app1_b_changed.lock"
    )
...
def test_multi_config_centralized(client_setup):
    ...
    assert "pkgb/0.1: DEP FILE pkgawin: HelloA" in c.out
    c.run(
        "create pkgb --name=pkgb --version=0.1 -s os=Linux "
