...
def test_pkg_config_name():
    ...
    module_path = os.path.join(
        client.current_folder, "conan-qbs-deps", "myfirstlib.json"
    )
    assert os.path.exists(module_path)


@pytest.mark.parametrize(
    "host_os, arch, build_type",
    [
        ("Linux", "x86_64", "Debug"),
...
def test_components():
    ...
    module1_path = os.path.join(
        client.current_folder, "conan-qbs-deps", "mycomponent.json"
    )
    ...
    module2_path = os.path.join(
        client.current_folder, "conan-qbs-deps", "mycomponent2.json"
    )
    ...
    main_module_path = os.path.join(
        client.current_folder, "conan-qbs-deps", "mylib.json"
    )
    ...
    assert main_module_content.get("dependencies") == [
        {"name": "mycomponent", "version": "0.1"},  # name overriden, version default
        {
            "name": "mycomponent2",
            "version": "19.8.199",
        },  # name default, version overriden
    ]
...
def test_cpp_info_requires():
    ...
    * third/0.4
    ...
    client.save(
        {
            "conanfile.py": GenConanfile("third", "0.3")
            .with_require("second/0.2")
    ...
    module_path = os.path.join(
        client2.current_folder, "conan-qbs-deps", "myfirstlib.json"
    )
    assert os.path.exists(module_path)
    module_content = json.loads(load(module_path))
    ...
    # second.mycomponent
    module_path = os.path.join(
        client2.current_folder, "conan-qbs-deps", "mycomponent.json"
    )
    assert os.path.exists(module_path)
    module_content = json.loads(load(module_path))
    ...
    # second.myfirstcomp
    module_path = os.path.join(
        client2.current_folder, "conan-qbs-deps", "myfirstcomp.json"
    )
    assert os.path.exists(module_path)
    module_content = json.loads(load(module_path))
...
def test_components_conflict():
    ...
    module_path = os.path.join(
        client.current_folder, "conan-qbs-deps", "mycoollib.json"
    )
    assert os.path.exists(module_path)
    module_content = json.loads(load(module_path))
