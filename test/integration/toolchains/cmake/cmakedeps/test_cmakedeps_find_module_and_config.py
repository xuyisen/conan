...
from conan.tools.cmake.cmakedeps.cmakedeps import (
    FIND_MODE_CONFIG,
    FIND_MODE_MODULE,
    FIND_MODE_BOTH,
    FIND_MODE_NONE,
...
@pytest.mark.parametrize(
    "cmake_find_mode",
    [FIND_MODE_CONFIG, FIND_MODE_MODULE, FIND_MODE_BOTH, FIND_MODE_NONE, None],
)
def test_reuse_with_modules_and_config(cmake_find_mode):
    ...
    if cmake_find_mode is not None:
        s = 'self.cpp_info.set_property("cmake_find_mode", "{}")'.format(
            cmake_find_mode
        )
    ...
    t.run(
        "install . --output-folder=install -s os=Linux -s compiler=gcc -s compiler.version=6 "
        "-s compiler.libcxx=libstdc++11"
    )

    def exists_config():
        return os.path.exists(
            os.path.join(t.current_folder, "install", "mydep-config.cmake")
        )

    def exists_module():
        return os.path.exists(
            os.path.join(t.current_folder, "install", "Findmydep.cmake")
        )
