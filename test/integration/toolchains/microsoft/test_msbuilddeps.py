...
def test_msbuilddeps_import_condition(
    build_type, arch, configuration, exp_platform, config_key, platform_key
):
    client = TestClient()
    ...
    app = (
        app.replace(f'ms.configuration_key = "{config_key}"', "")
        if config_key is None
        else app
    )
    app = (
        app.replace(f'ms.platform_key = "{platform_key}"', "")
    ...
    client.save(
        {
            ...
            "lib/conanfile.py": GenConanfile("lib", "0.1").with_package_type(
                "header-library"
            ),
        }
    )
    ...
    client.run(
        f'install app -s build_type={build_type} -s arch={arch} -o *:platform={exp_platform} -o *:configuration="{configuration}"'
    )
...
def test_msbuilddeps_format_names():
    ...
    c.run(
        "install --require=pkg.name-more+/1.0@ -g MSBuildDeps -s build_type=Release -s arch=x86_64"
    )
    ...
    libmpdecimal_release = c.load(
        "conan_pkg_name-more__libmpdecimal___release_x64.props"
    )
    assert (
        "$(conan_pkg_name-more__mycomp_some-comp__props_imported)"
        in libmpdecimal_release
    )
...
class TestMSBuildDepsSkips:
    ...
    def test_msbuilddeps_skipped_deps(self):
        ...
        assert not os.path.exists(
            os.path.join(c.current_folder, "app", "conan_liba.props")
        )
...
@pytest.mark.skipif(
    platform.system() != "Windows", reason="MSBuildDeps broken with POSIX paths"
)
...
def test_msbuilddeps_relocatable(withdepl):
    ...
    c.save(
        {
            "libh/conanfile.py": GenConanfile("libh", "0.1").with_package_type(
                "header-library"
            ),
            "libs/conanfile.py": GenConanfile("libs", "0.2")
            .with_package_type("static-library")
            .with_requires("libh/0.1"),
            "libd/conanfile.py": GenConanfile("libd", "0.3").with_package_type(
                "shared-library"
            ),
            "app/conanfile.py": GenConanfile()
            .with_requires("libh/0.1", "libs/0.2", "libd/0.3")
            .with_settings("arch", "build_type"),
        }
    )
