...
class TestPerPackageSetting:
    def test_per_package_setting(self):
        ...
        client.run(
            "create . --name=pkg --version=0.1 --user=user --channel=testing -s os=Windows"
        )
        client.save(
            {"conanfile.py": GenConanfile().with_require("pkg/0.1@user/testing")}
        )
        client.run(
            "create . --name=consumer --version=0.1 --user=user --channel=testing -s os=Linux -s pkg*:os=Windows"
        )
    ...
    def test_per_package_setting_build_type(self):
        ...
        client.run(
            "create . -s *:os=Windows -s *-model*:build_type=Debug -s build_type=Release"
        )
        assert "pkg-model/0.1: BUILDTYPE Windows:Debug!!!!" in client.out

        client.save(
            {
                "conanfile.py": GenConanfile("consumer", "0.1").with_require(
                    "pkg-model/0.1"
                )
            }
        )
        client.run(
            "create . -s *:os=Linux -s *-model*:os=Windows "
    ...
    def test_per_package_setting_no_userchannel(self):
        ...
        client.run(
            "create . --name=consumer --version=0.1 -s os=Linux -s pkg*:os=Windows"
        )
        assert "consumer/0.1: Created package" in client.out
    ...
    def test_per_package_subsetting(self):
        ...
        client.run(
            "create . --name=pkg --version=0.1 --user=user --channel=testing %s  -s compiler.libcxx=libstdc++11"
            % settings
        )
        client.save(
            {"conanfile.py": GenConanfile().with_require("pkg/0.1@user/testing")}
        ...
        client.run(
            "create . --name=consumer --version=0.1 --user=user --channel=testing %s -s compiler.libcxx=libstdc++ "
    ...
    def test_per_package_setting_all_packages_without_user_channel(self):
        ...
        client.run(
            "create . --name=pkg3 --version=0.1 --user=user --channel=channel -s os=Linux"
        )
        client.save(
            {
                "conanfile.py": GenConanfile().with_requires(
                    "pkg1/0.1", "pkg2/0.1@user", "pkg3/0.1@user/channel"
                )
            }
        )
...
def test_per_package_settings_target():
    ...
    c.run(
        "install --tool-requires=gcc/0.1 -s:b arch=x86_64 -s arch=armv8 -s:h gcc*:arch=armv7"
        " --format=json"
    )
...
def test_per_package_settings_build():
    ...
    c.run(
        "install --tool-requires=cmake/0.1 -s:b build_type=Release -s:b cmake*:build_type=Debug "
        "--build=missing"
    )
...
def test_package_settings_mixed_patterns():
    ...
    conanfile = textwrap.dedent("""
        ...
        class Pkg(ConanFile):
            ...
            """)
    c.save({"profile": profile, "conanfile.py": conanfile})
