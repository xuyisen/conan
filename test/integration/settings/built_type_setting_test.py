...
"""
        ...
        client.run(
            "install --requires=pkg/0.1@lasote/testing -pr=myprofile --build='*'"
        )
        ...
        client.run(
            "create . --name=pkg --version=0.1 --user=lasote --channel=testing -pr=myprofile"
        )
        ...
        assert (
            "ERROR: Invalid setting 'None' is not a valid 'settings.build_type'"
            in client.out
        )
        assert (
