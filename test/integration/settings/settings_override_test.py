...
def test_override(client):
    client.run(
        "install --requires=visual/0.1@lasote/testing --build missing -s compiler=msvc "
...
def test_exclude_patterns_settings():
    ...
    client.run(
        "install consumer -s build_type=Debug "
    ...
    client.run(
        "create consumer --name=consumer --version=1.0 "
    ...
    client.run(
        "install --reference consumer/1.0 -s build_type=Debug", assert_error=True
    )
    # Pre-check, there is no Debug package for any of them
    assert (
        "ERROR: Missing prebuilt package for 'consumer/1.0', 'openssl/1.0', 'zlib/1.0'"
    )
    ...
    client.run(
        "install --requires consumer/1.0 -s zlib/*:build_type=Debug -s *:build_type=Release"
    )

    # Try to install with this two scoped conditions, This is ERROR the right side has priority
    client.run(
        "install --requires consumer/1.0 -s *:build_type=Release -s zlib/*:build_type=Debug",
        assert_error=True,
    )
    ...
    client.run(
        "install --requires consumer/1.0 -s *:build_type=Release "
    ...
    client.run(
        "install --requires consumer/1.0 -s !zlib:build_type=Debug "
