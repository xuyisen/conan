...
class HelloConan(ConanFile):
    ...
    def test_runtime_auto(self):
        ...
        self.client.run(
            "install --requires=hello0/0.1@lasote/channel --build missing -s build_type=Debug"
        )
    ...
    def test_runtime_not_present_ok(self):
        ...
        self.client.run(
            "install --requires=hello0/0.1@lasote/channel --build missing "
            '-s compiler.runtime="dynamic"',
            assert_error=True,
        )
        self.assertIn(
            "'settings.compiler.runtime' doesn't exist for 'msvc'", self.client.out
        )
