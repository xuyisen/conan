...
def test_rosenv():
    ...
    conanfile3 = textwrap.dedent("""
        ...
        ROSEnv
        """)
    client.save({"conanfile3.txt": conanfile3})
    ...
    toolchain_path = os.path.join(
        client.current_folder, "install", "conan", "conan_toolchain.cmake"
    )
...
def test_rosenv_shared_libraries():
    ...
    client = TestClient()
    c1 = (
        GenConanfile("lib1", "1.0")
        .with_shared_option(False)
        .with_package_file("lib/lib1", "lib-content")
    )
    c2 = (
        GenConanfile("lib2", "1.0")
        .with_shared_option(False)
        .with_requirement("lib1/1.0")
        .with_package_file("lib/lib2", "lib-content")
    )
    c3 = textwrap.dedent("""
           [requires]
           lib2/1.0
           ...
           ROSEnv
           """)
    client.save({"conanfile1.py": c1, "conanfile2.py": c2, "conanfile3.txt": c3})
    ...
    conanrosenv_path = os.path.join(
        client.current_folder, "install", "conan", "conanrosenv.sh"
    )
    client.run_command(f'. "{conanrosenv_path}" && env')
