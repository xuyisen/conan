...
def test_package_manager_platform(platform, tool):
    with mock.patch("platform.system", return_value=platform):
        with mock.patch("distro.id", return_value=""):
            with mock.patch(
                "conan.ConanFile.context", new_callable=PropertyMock
            ) as context_mock:
...
@pytest.mark.parametrize(
    "sudo, sudo_askpass, expected_str",
    [
        (True, True, "sudo -A "),
        (True, False, "sudo "),
        (False, True, ""),
...
@pytest.mark.parametrize(
    "tool_class", [Apk, Apt, Yum, Dnf, Brew, Pkg, PkgUtil, Chocolatey, PacMan, Zypper]
)
def test_tools_install_mode_check(tool_class):
    ...
    with mock.patch(
        ...
        with pytest.raises(ConanException) as exc_info:
            ...
            with patch.object(
                _SystemPackageManagerTool, "check", MagicMock(side_effect=fake_check)
            ):
                tool.install(["package1", "package2"])
        assert (
            exc_info.value.args[0]
            == "System requirements: 'package1, package2' are missing but "
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            Apt,
            "x86",
            "apt-get install -y --no-install-recommends package1:i386 package2:i386",
        ),
...
def test_tools_install_mode_install_to_build_machine_arch(
    tool_class, arch_host, result
):
    conanfile = ConanFileMock()
    conanfile.settings = MockSettings({"arch": arch_host})
