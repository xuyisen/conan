...
def test_exports():
    ...
    c.save({"conanfile.py": conanfile, "hello.h": "hello", "data.txt": "data"})
    c.run("create .")
...
def test_test_package_copied():
    ...
    client.save({"conanfile.py": conanfile, "test_package/foo.txt": "bar"})
...
def test_source_changes_generate_new_revisions():
    ...
    tc.save(
        {
            "conanfile.py": GenConanfile("lib", "1.0").with_exports_sources("file.h"),
            "file.h": "Hello World!",
        }
    )
