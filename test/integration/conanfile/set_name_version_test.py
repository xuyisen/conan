...
class SetVersionNameTest(unittest.TestCase):
    ...
    def test_set_version_name(self, user_channel):
        ...
        self.assertIn("pkg/2.1%s: Exported" % user_channel, client.out)
        # installing it doesn't break
        client.run(
            "install --requires=pkg/2.1%s --build=missing" % (user_channel or "@")
        )
        client.assert_listed_require({f"pkg/2.1{user_channel}": "Cache"})
        client.assert_listed_binary(
            {f"pkg/2.1{user_channel}": (NO_SETTINGS_PACKAGE_ID, "Build")}
        )
        ...
        client.assert_listed_require({f"pkg/2.1{user_channel}": "Cache"})
        client.assert_listed_binary(
            {f"pkg/2.1{user_channel}": (NO_SETTINGS_PACKAGE_ID, "Cache")}
        )
    ...
    def test_set_version_name_file(self):
        ...
        conanfile = textwrap.dedent("""
            ...
            """)
        client.save(
            {"conanfile.py": conanfile, "name.txt": "pkg", "version.txt": "2.1"}
        )
    ...
    def test_set_version_name_crash(self):
        ...
        self.assertIn(
            "ERROR: conanfile.py: Error in set_name() method, line 5", client.out
        )
        ...
        client.run("export .", assert_error=True)
        self.assertIn(
            "ERROR: conanfile.py: Error in set_version() method, line 5", client.out
        )
        self.assertIn("name 'error' is not defined", client.out)
    ...
    conanfile = textwrap.dedent("""
        ...
        class Lib(ConanFile):
            ...
            def set_version(self):
                self.version = load(self, "version.txt")
        """)
    c.save({"conanfile.py": conanfile, "version.txt": "1.0\n"})
    c.run("export .", assert_error=True)
    assert "ERROR: Invalid package version '1.0" in c.out
