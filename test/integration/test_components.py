...
def test_components_cycles():
    ...
    out = c.out
    assert (
        "ERROR: Error in generator 'CMakeDeps': error generating context for 'testcycle/1.0': "
...
def test_components_not_required():
    ...
    c.save(
        {
            "expat/conanfile.py": GenConanfile("expat", "1.0"),
...
def test_components_overrides():
    ...
    c.save(
        {
            "libffi/conanfile.py": GenConanfile("libffi"),
            "glib/conanfile.py": GenConanfile("glib", "2.76.2").with_requires(
                "libffi/3.0"
            ),
...
def test_duplication_component_properties():
    ...
    tc.save({"dep/conanfile.py": dep, "conanfile.py": conanfile})
    tc.run("create dep")
    tc.run("create .")
