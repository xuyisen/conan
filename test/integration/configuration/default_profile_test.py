...
class DefaultProfileTest(unittest.TestCase):
    def test_conanfile_txt_incomplete_profile(self):
        ...
        self.assertIn(
            "pkg/0.1@lasote/stable: Package '%s' created" % NO_SETTINGS_PACKAGE_ID,
            client.out,
        )
...
def test_conf_default_two_profiles():
    client = TestClient()
    save(
        os.path.join(client.paths.profiles_path, "mydefault"), "[settings]\nos=FreeBSD"
    )
    save(
        os.path.join(client.paths.profiles_path, "mydefault_build"),
        "[settings]\nos=Android",
    )
