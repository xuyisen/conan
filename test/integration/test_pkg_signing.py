...
def test_pkg_sign():
    ...
    c.save(
        {
            ...
            .with_exports("export/*")
            .with_exports_sources("export_sources/*")
            .with_package_file("myfile", "mycontents!"),
    ...
    assert (
        "Signing files:  ['conan_export.tgz', 'conan_sources.tgz', "
    ...
    assert "Verifying ref:  pkg/0.1" in c.out
    assert (
        "VERIFYING  conanfile.py" not in c.out
    )  # It doesn't re-verify previous contents
