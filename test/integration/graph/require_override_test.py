...
class TestRequireOverride:
    def test_override_user_channel(self):
        ...
        c.save(
            {
                ...
                .with_requirement("dep1/0.1@us/chan", override=True)
        ...
        c.assert_overrides(
            {"dep1/0.1": ["dep1/0.1@us/chan"], "dep2/0.1@us/chan": ["dep2/0.1"]}
        )
        c.assert_listed_require({"dep1/0.1@us/chan": "Cache", "dep2/0.1": "Cache"})
    ...
    def test_can_override_even_versions_with_build_metadata(self):
        ...
        c.assert_listed_require({"lib/1.0+xyz": "Cache", "pkg/1.0": "Cache"})
