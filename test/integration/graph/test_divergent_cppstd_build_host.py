...
def test_divergent_cppstd_build_host():
    ...
    c.save(
        {
            ...
            "rainbow/conanfile.py": GenConanfile("rainbow", "1.0")
    ...
    c.run(
        "install . --build=missing -s compiler.cppstd=14 -s:b compiler.cppstd=17 --format=json",
    ...
    assert (
        graph["graph"]["nodes"]["1"]["package_id"]
