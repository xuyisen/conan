...
class TestPlatformRequires:
    ...
    def test_platform_requires_range_non_matching(self):
        ...
        client.save(
            {
                "dep/conanfile.py": GenConanfile("dep", "1.1"),
                "conanfile.py": GenConanfile("pkg", "1.0").with_requires("dep/[>=1.0]"),
                "profile": "[platform_requires]\ndep/0.1",
            }
        )
    ...
    def test_platform_requires_no_host(self):
        ...
        client.save(
            {
                "conanfile.py": GenConanfile("pkg", "1.0").with_tool_requires(
                    "dep/1.0"
                ),
                "profile": "[platform_requires]\ndep/1.0",
            }
    ...
    def test_platform_requires_build_context(self):
        ...
        client.save(
            {
                "tool/conanfile.py": GenConanfile("tool", "1.0").with_requires(
                    "dep/1.0"
                ),
                "pkg/conanfile.py": GenConanfile("pkg", "1.0").with_tool_requires(
                    "tool/1.0"
                ),
                "profile": "[settings]\nos=Linux\n[platform_requires]\ndep/1.0",
            }
    ...
    def test_consumer_resolved_version(self):
        ...
        conanfile = textwrap.dedent("""
            ...
            class Pkg(ConanFile):
                ...
                """)
        client.save(
            {"conanfile.py": conanfile, "profile": "[platform_requires]\ndep/1.1"}
        )
    ...
    def test_platform_requires_with_options(self):
        ...
        conanfile = GenConanfile("pkg", "1.0").with_requirement(
            "dep/1.0", options={"dep/1.0:myoption": True}
        )
        client.save(
            {"conanfile.py": conanfile, "profile": "[platform_requires]\ndep/1.0"}
        )
        client.run(
            "create . -pr=profile"
        )  # This crashed before for non-existing options
        assert "dep/1.0 - Platform" in client.out
...
class TestPlatformTestRequires:
    def test_platform_requires(self):
        ...
        client.save(
            {
                "conanfile.py": GenConanfile("pkg", "1.0").with_test_requires(
                    "dep/1.0"
                ),
                "profile": "[platform_requires]\ndep/1.0",
            }
...
class TestGenerators:
    def test_platform_requires_range(self):
        ...
        assert not os.path.exists(
            os.path.join(client.current_folder, "dep-config.cmake")
        )
...
class TestPackageID:
    ...
    @pytest.mark.parametrize(
        "package_id_mode", ["recipe_revision_mode", "full_package_mode"]
    )
    def test_package_id_modes(self, package_id_mode):
        ...
        client = TestClient(light=True)
        client.save_home(
            {"global.conf": f"core.package_id:default_unknown_mode={package_id_mode}"}
        )
        client.save(
            {
                "conanfile.py": GenConanfile("pkg", "1.0").with_requires("dep/1.0"),
    ...
    def test_package_id_explicit_revision(self):
        ...
        assert (
            "pkg/1.0#7ed9bbd2a7c3c4381438c163c93a9f21:"
