...
class TestVersionRangeOverride:
    ...
    def _setup(self):
        ...
        self.t.save(
            {
                "libb/conanfile.py": GenConanfile(),
                "libc/conanfile.py": GenConanfile().with_require(
                    "libb/[<=2.0]@user/channel"
                ),
            }
        )
        self.t.run(
            "export libb --name=libb --version=1.0 --user=user --channel=channel"
        )
        self.t.run(
            "export libb --name=libb --version=2.0 --user=user --channel=channel"
        )
        self.t.run(
            "export libb --name=libb --version=3.0 --user=user --channel=channel"
        )
        self.t.run(
            "export libc --name=libc --version=1.0 --user=user --channel=channel"
        )

    def test(self):
        # Use the version range
        self.t.save(
            {"conanfile.py": GenConanfile().with_require("libc/1.0@user/channel")}
        )
        ...
        self.t.save(
            {
                ...
                .with_requirement("libb/3.0@user/channel", override=True)
                .with_require("libc/1.0@user/channel")
            }
        )
        self.t.run("graph info . --filter requires")
        self.t.assert_overrides(
            {"libb/[<=2.0]@user/channel": ["libb/3.0@user/channel"]}
        )
    ...
    def test_override_using_version_range(self):
        ...
        self.t.save(
            {
                "conanfile.py": GenConanfile()
                .with_requirement("libb/[<2.x]@user/channel", override=True)
                .with_require("libc/1.0@user/channel")
            }
        )
        self.t.run("graph info . --filter requires")
        self.t.assert_overrides(
            {"libb/[<=2.0]@user/channel": ["libb/[<2.x]@user/channel"]}
        )
        assert "libb/2.0@user/channel" in self.t.out
...
class TestVersionRangeOverrideFail:
    def test_override(self):
        ...
        t.run(
            "create . --name=ros_core --version=1.1.4 --user=3rdparty --channel=unstable"
        )
        t.run(
            "create . --name=ros_core --version=pr-53 --user=3rdparty --channel=snapshot"
        )
        t.save(
            {
        ...
        t.run(
            "create . --name=ros_perception --version=1.1.4 --user=3rdparty --channel=unstable"
        )
        t.run(
            "create . --name=ros_perception --version=pr-53 --user=3rdparty --channel=snapshot"
        )
        t.save(
            {
                "conanfile.py": GenConanfile().with_requires(
        ...
        t.save(
            {
                "conanfile.py": GenConanfile("pkga", "0.1").with_requires(
        ...
        t.save(
            {
                "conanfile.py": GenConanfile("pkga", "0.1")
        ...
        expected_overrides = {
            "ros_core/[~1.1]@3rdparty/unstable": ["ros_core/pr-53@3rdparty/snapshot"],
            "ros_core/1.1.4@3rdparty/unstable": ["ros_core/pr-53@3rdparty/snapshot"],
        }
