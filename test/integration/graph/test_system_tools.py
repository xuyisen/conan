...
class TestToolRequires:
    ...
    def test_system_tool_require_range_non_matching(self):
        ...
        client.save(
            {
                "tool/conanfile.py": GenConanfile("tool", "1.1"),
                "conanfile.py": GenConanfile("pkg", "1.0").with_tool_requires(
                    "tool/[>=1.0]"
                ),
                "profile": "[platform_tool_requires]\ntool/0.1",
    ...
    def test_system_tool_require_no_host(self):
        ...
        client.save(
            {
                "conanfile.py": GenConanfile("pkg", "1.0").with_requires("tool/1.0"),
                "profile": "[platform_tool_requires]\ntool/1.0",
            }
        )
    ...
    def test_consumer_resolved_version(self):
        ...
        conanfile = textwrap.dedent("""
            ...
            class Pkg(ConanFile):
                ...
                """)
        client.save(
            {"conanfile.py": conanfile, "profile": "[platform_tool_requires]\ntool/1.1"}
        )
    ...
    def test_require_build_context(self):
        ...
        c.save(
            {
                "autoconf/conanfile.py": GenConanfile("autoconf", "0.1"),
                "automake/conanfile.py": GenConanfile("automake", "0.1").with_requires(
                    "autoconf/0.1"
                ),
                "libtool/conanfile.py": GenConanfile("libtool", "0.1")
                .with_requires("automake/0.1")
                .with_tool_requires("automake/0.1"),
                "libcurl/conanfile.py": GenConanfile(
                    "libcurl", "0.1"
                ).with_tool_requires("libtool/0.1"),
...
class TestGenerators:
    def test_system_tool_require_range(self):
        ...
        assert not os.path.exists(
            os.path.join(client.current_folder, "tool-config.cmake")
        )
...
class TestPackageID:
    ...
    def test_package_id_modes(self, package_id_mode):
        ...
        client = TestClient(light=True)
        client.save_home(
            {"global.conf": f"core.package_id:default_build_mode={package_id_mode}"}
        )
        client.save(
            {
                "conanfile.py": GenConanfile("pkg", "1.0").with_tool_requires(
    ...
    def test_package_id_explicit_revision(self):
        ...
        client = TestClient(light=True)
        client.save_home(
            {"global.conf": "core.package_id:default_build_mode=recipe_revision_mode"}
        )
        client.save(
            {
                "conanfile.py": GenConanfile("pkg", "1.0").with_tool_requires(
        ...
        assert (
            "pkg/1.0#27a56f09310cf1237629bae4104fe5bd:"
