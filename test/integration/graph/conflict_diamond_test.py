...
class TestConflictDiamondTest:
    def test_version_diamond_conflict(self):
        ...
        c.save(
            {
                ...
                "ai/conanfile.py": GenConanfile("ai", "1.0").with_requires(
                    "math/1.0.1"
                ),
        ...
        for v in ("1.0", "1.0.1", "1.0.2"):
            ...
            c.run("install game")
            c.assert_overrides({"math/1.0": [f"math/{v}"], "math/1.0.1": [f"math/{v}"]})
...
def test_conflict_user(version_range):
    ...
    assert (
        f"Version conflict: Conflict between dep/{v}@user1 and dep/{v}@user2" in c.out
    )
...
def test_conflict_user_order():
    ...
    c.run("install app", assert_error=True)
    assert (
        "ERROR: Version conflict: Conflict between dep/1.0@user1 and dep/[>=1.0]@user2"
        in c.out
    )
...
class TestErrorVisibleFalse:
    def test_subgraph_conflict(self, order, test):
        ...
        if order:
            pkg1.with_requirement(
                "pkg3/1.0", visible=False, test=test
            ).with_requirement("pkg2/1.0")
        else:
            pkg1.with_requirement("pkg2/1.0").with_requirement(
                "pkg3/1.0", visible=False, test=test
            )
        tc.save(
            {
                "pkg3/conanfile.py": GenConanfile("pkg3"),
                "pkg2/conanfile.py": GenConanfile("pkg2", "1.0").with_requirement(
                    "pkg3/1.1"
                ),
        ...
        assert (
            "ERROR: Version conflict: Conflict between pkg3/1.1 and pkg3/1.0" in tc.out
        )
    ...
    def test_subgraph_conflict_second_level(self, order, test):
        ...
        if order:
            pkg.with_requirement(
                "gtest/1.0", visible=False, test=test
            ).with_requirement("boost/1.0")
        else:
            pkg.with_requirement("boost/1.0").with_requirement(
                "gtest/1.0", visible=False, test=test
            )
        tc.save(
            {
                "zlib/conanfile.py": GenConanfile("zlib"),
                "boost/conanfile.py": GenConanfile("boost", "1.0").with_requirement(
                    "zlib/1.1"
                ),
                "gtest/conanfile.py": GenConanfile("gtest", "1.0").with_requirement(
                    "zlib/1.0"
                ),
        ...
        if order:
            assert (
                "ERROR: Version conflict: Conflict between zlib/1.1 and zlib/1.0"
                in tc.out
            )
            assert "Conflict originates from boost/1.0" in tc.out
        else:
            assert (
                "ERROR: Version conflict: Conflict between zlib/1.0 and zlib/1.1"
                in tc.out
            )
    ...
    def test_subgraph_no_conflict(self, order, test):
        ...
        assert (
            "WARN: risk: Packages required both with visible=True and visible=False"
            in tc.out
        )
    ...
    def test_transitive_conflict(self, order, test):
        ...
        tc.run("export pkg1")
        tc.run(
            "graph info --requires=pkg3/1.1 --requires=pkg1/1.0 --format=html",
            assert_error=True,
        ...
        if order:
            assert (
                "ERROR: Version conflict: Conflict between pkg3/1.1 and pkg3/1.0"
                in tc.out
            )
        else:
            assert (
                "ERROR: Version conflict: Conflict between pkg3/1.0 and pkg3/1.1"
                in tc.out
            )
    ...
    def test_transitive_conflict_second_level(self, order, test):
        ...
        tc.run("export pkg1")
        tc.run(
            "graph info --requires=zlib/1.1 --requires=pkg1/1.0 --format=html",
            assert_error=True,
            redirect_stdout="graph.html",
        )
        # Check that the graph.html is generated
    ...
    def test_transitive_version_range_no_conflict_second_level(self, order, test):
        ...
        assert (
            "pkg1/1.0: WARN: risk: This package has 2 different dependencies"
            not in tc.out
        )
    ...
    def test_transitive_orphans(self, order, test):
        ...
        tc.save(
            {
                "pkg4/conanfile.py": GenConanfile("pkg4", "0.1"),
