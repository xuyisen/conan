...
class TestValidate:
    def test_validate_create(self):
        ...
        assert (
            "pkg/0.1: Package '9a4eb3c8701508aa9458b1a73d0633783ecc2270' created"
            in client.out
        )

        error = client.run(
            "create . --name=pkg --version=0.1 -s os=Windows", assert_error=True
        )
        ...
        assert (
            myjson["graph"]["nodes"]["1"]["info_invalid"]
    ...
    def test_validate_header_only(self):
        ...
        client.run(
            "create . --name pkg --version=0.1 -s os=Linux -s compiler=gcc "
            "-s compiler.version=11 -s compiler.libcxx=libstdc++11"
        )
        assert re.search(r"Package '(.*)' created", str(client.out))

        client.run(
            "create . --name pkg --version=0.1 -o header_only=False -s os=Linux "
        ...
        client.run(
            "create . --name pkg --version=0.1  -o header_only=False -s os=Macos "
        ...
        assert (
            "Invalid: Current cppstd (98) is lower than the required C++ "
            "standard (11)" in client.out
        )

        client.run(
            "create . --name pkg --version=0.1  -o header_only=False -o shared=True "
    ...
    def test_validate_compatible(self):
        ...
        assert (
            "pkg/0.1: Cannot build for this configuration: Windows not supported"
            in client.out
        )
        client.assert_listed_binary({"pkg/0.1": (missing_id, "Invalid")})

        client.run(
            "install --requires=pkg/0.1@ -s os=Windows --build=pkg*", assert_error=True
        )
        assert (
            "pkg/0.1: Cannot build for this configuration: Windows not supported"
            in client.out
    ...
    def test_validate_remove_package_id_create(self):
        ...
        client.run("create . --name=pkg --version=0.1 -s os=Linux")
        assert (
            "pkg/0.1: Package '{}' created".format(NO_SETTINGS_PACKAGE_ID) in client.out
        )

        client.run("create . --name=pkg --version=0.1 -s os=Windows", assert_error=True)
        assert "pkg/0.1: Invalid: Windows not supported" in client.out
        client.assert_listed_binary(
            {"pkg/0.1": ("da39a3ee5e6b4b0d3255bfef95601890afd80709", "Invalid")}
        )
    ...
    def test_validate_compatible_also_invalid(self):
        ...
        client.run(
            "create . --name=pkg --version=0.1 -s os=Linux -s build_type=Release"
        )
        ...
        error = client.run(
            "create . --name=pkg --version=0.1 -s os=Windows -s build_type=Release",
            assert_error=True,
        )
    ...
    def test_validate_compatible_also_invalid_fail(self):
        ...
        error = client.run(
            "install --requires=pkg/0.1@ -s os=Windows -s build_type=Release",
            assert_error=True,
        )
        assert error == ERROR_INVALID_CONFIGURATION
        assert "pkg/0.1: Invalid: Windows not supported" in client.out
        ...
        error = client.run(
            "install --requires=pkg/0.1@ -s os=Windows -s build_type=Debug",
            assert_error=True,
        )
        assert error == ERROR_INVALID_CONFIGURATION
        assert "pkg/0.1: Invalid: Windows not supported" in client.out
        ...
        error = client.run(
            "create . --name=pkg --version=0.1 -s os=Windows -s build_type=Debug",
            assert_error=True,
        )
        assert error == ERROR_INVALID_CONFIGURATION
        assert "pkg/0.1: Invalid: Windows not supported" in client.out
    ...
    def test_validate_options(self):
        ...
        client.save(
            {"conanfile.py": GenConanfile().with_requires("pkg2/0.1", "pkg1/0.1")}
        )
    ...
    def test_validate_package_id_mode(self):
        client = TestClient()
        client.save_home(
            {"global.conf": "core.package_id:default_unknown_mode=full_package_mode"}
        )
        conanfile = textwrap.dedent("""
          from conan import ConanFile
          from conan.errors import ConanInvalidConfiguration
        ...
        assert error == ERROR_INVALID_CONFIGURATION
        client.assert_listed_binary(
            {"dep/0.1": ("ebec3dc6d7f6b907b3ada0c3d3cdc83613a2b715", "Invalid")}
        )
        client.assert_listed_binary(
            {"pkg/0.1": ("19ad5731bb09f24646c81060bd7730d6cb5b6108", "Build")}
        )
    ...
    def test_validate_export_pkg(self):
        ...
        assert (
            "ERROR: conanfile.py (test/1.0): Invalid ID: Invalid: never ever" in c.out
        )
    ...
    def test_validate_build_export_pkg(self):
        ...
        c.run("export-pkg . --name=test --version=1.0", assert_error=True)
        assert (
            "conanfile.py (test/1.0): Cannot build for this configuration: never ever"
            in c.out
        )
...
class TestValidateCppstd:
    ...
    def test_build_17_consume_14(self):
        ...
        client.assert_listed_binary(
            {"pkg/0.1": ("91faf062eb94767a31ff62a46767d3d5b41d1eff", "Build")}
        )
        # create with cppstd=14 fails, not enough
        client.run(f"create . {settings} -s compiler.cppstd=14", assert_error=True)
        client.assert_listed_binary(
            {"pkg/0.1": ("36d978cbb4dc35906d0fd438732d5e17cd1e388d", "Invalid")}
        )
        assert (
            "pkg/0.1: Cannot build for this configuration: I need at least cppstd=17 to build"
            in client.out
        )
        ...
        client.assert_listed_binary(
            {"pkg/0.1": ("91faf062eb94767a31ff62a46767d3d5b41d1eff", "Cache")}
        )

        # install with not enough cppstd should fail
        client.run(
            f"install --requires=pkg/0.1@ {settings} -s compiler.cppstd=11",
            assert_error=True,
        )
        # not even trying to fallback to compatibles
        assert "pkg/0.1: Checking" not in client.out
        client.assert_listed_binary(
            {"pkg/0.1": ("8415595b7485d90fc413c2f47298aa5fb05a5468", "Invalid")}
        )
    ...
    def test_header_only_14(self):
        ...
        assert (
            "Current cppstd (11) is lower than the required C++ standard (14)"
            in client.out
        )
    ...
    def test_build_17_consume_14_transitive(self):
        ...
        client.assert_listed_binary(
            {"pkg/0.1": ("91faf062eb94767a31ff62a46767d3d5b41d1eff", "Skip")}
        )
        ...
        client.assert_listed_binary(
            {"engine/0.1": ("dc24e2caf6e1fa3e8bb047ca0f5fa053c71df6db", "Missing")}
        )
        client.run(
            f"install app {settings} -s compiler.cppstd=11 --build=missing",
            assert_error=True,
        )
    ...
    def test_build_17_consume_14_transitive_erasure(self):
        ...
        client.run(f"create engine {settings} -s compiler.cppstd=17")
        client.assert_listed_binary(
            {"engine/0.1": ("493976208e9989b554704f94f9e7b8e5ba39e5ab", "Build")}
        )
        client.assert_listed_binary(
            {"pkg/0.1": ("91faf062eb94767a31ff62a46767d3d5b41d1eff", "Cache")}
        )
        client.run(f"install app {settings} -s compiler.cppstd=17 -v")
        client.assert_listed_binary(
            {"engine/0.1": ("493976208e9989b554704f94f9e7b8e5ba39e5ab", "Cache")}
        )
        client.assert_listed_binary(
            {"pkg/0.1": ("91faf062eb94767a31ff62a46767d3d5b41d1eff", "Skip")}
        )
        ...
        client.assert_listed_binary(
            {"pkg/0.1": ("8415595b7485d90fc413c2f47298aa5fb05a5468", "Skip")}
        )
        client.assert_listed_binary(
            {"engine/0.1": ("493976208e9989b554704f94f9e7b8e5ba39e5ab", "Cache")}
        )
        client.run(
            f"install app {settings} -s compiler.cppstd=11 --build=engine*",
            assert_error=True,
        )
        assert "pkg/0.1: Invalid: I need at least cppstd=14 to be used" in client.out
    ...
    def test_exact_cppstd(self, use_attribute):
        ...
        settings = "-s compiler=gcc -s compiler.version=9 -s compiler.libcxx=libstdc++"
        client.run(
            f"create . --name=pkg --version=0.1 {settings} -s compiler.cppstd=17"
        )
        client.assert_listed_binary(
            {"pkg/0.1": ("91faf062eb94767a31ff62a46767d3d5b41d1eff", "Build")}
        )
        ...
        client.run(
            f"install --requires=pkg/0.1 {settings} -s compiler.cppstd=14",
            assert_error=True,
        )
        ...
        client.run(
            f"install --requires=pkg/0.1 {settings} -s compiler.cppstd=14 --build=missing"
        )
        client.assert_listed_binary(
            {"pkg/0.1": ("36d978cbb4dc35906d0fd438732d5e17cd1e388d", "Build")}
        )
        assert "compiler.cppstd=14" in client.out
        assert "compiler.cppstd=17" not in client.out
    ...
    def test_extension_properties_make_transitive(self):
        ...
        compat_path = os.path.join(
            tc.cache_folder, "extensions/plugins/compatibility/compatibility.py"
        )
        compat_contents = load(compat_path)

        transitive_expansions = textwrap.indent(
            textwrap.dedent("""
        ...
        compat_contents = compat_contents.replace(
            'extension_properties = getattr(conanfile, "extension_properties", {})',
            transitive_expansions,
        )
...
class TestCompatibleSettingsTarget:
    ...
    def test_settings_target_in_compatibility_method_within_recipe(self):
        ...
        client.run(
            "create . --name=tool --version=0.1 -s os=Linux -s:h arch=armv6 --build-require"
        )
    ...
    def test_no_settings_target_in_recipe_but_in_compatibility_method(self):
        ...
        client.save({"conanfile.py": app_conanfile})
        error = client.run(
            "create . --name=app --version=0.1 -s os=Linux -s:h arch=armv7",
            assert_error=True,
        )
    ...
    def test_three_packages_with_and_without_settings_target(self):
        ...
        client.run(
            "create . --name=tool_a --version=0.1 -s os=Linux -s:h arch=armv6 -s:b arch=x86_64 --build-require"
        )
        ...
        client.run(
            "create . --name=tool_b --version=0.1 -s os=Linux -s arch=x86_64 -s:b arch=x86_64"
        )
        ...
        client.save({"conanfile.py": app_conanfile})
        client.run(
            "create . --name=app --version=0.1 -s os=Linux -s:h arch=armv7 -s:b arch=x86_64"
        )
    ...
    def test_settings_target_in_compatibility_method_within_recipe_package_info(self):
        ...
        client.save({"conanfile.py": tool_conanfile})
        client.run(
            "create . --name=tool --version=0.1 -s os=Linux -s:b arch=armv6 --build-require"
        )
        package_id = client.created_package_id("tool/0.1")
        assert f"tool/0.1: Package '{package_id}' created" in client.out
