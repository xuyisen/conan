...
def test_package_id_confs_header_only():
    ...
    client.save({"conanfile.py": conanfile, "profile": profile})
    client.run(
        'create . --name=pkg --version=0.1 -pr profile -c tools.build:cxxflags=["--flag1"]'
    )
    client.assert_listed_binary({"pkg/0.1": (NO_SETTINGS_PACKAGE_ID, "Build")})
    client.run("list *:*")
    assert "tools.build:cxxflags" not in client.out
    client.run(
        'create . --name=pkg --version=0.1 -pr profile -c tools.build:cxxflags=["--flag2"]'
    )
    client.assert_listed_binary({"pkg/0.1": (NO_SETTINGS_PACKAGE_ID, "Build")})
    client.run("list *:*")
    assert "tools.build:cxxflags" not in client.out
...
def test_conf_pkg_id_user_pattern_not_defined():
    ...
    tc.run("create lib")
    assert (
        "lib/1.0: Package 'da39a3ee5e6b4b0d3255bfef95601890afd80709' created" in tc.out
    )
...
def test_package_id_order(conf, pkgconf):
    ...
    tc.assert_listed_binary(
        {"pkg/1.0": ("43227c40b8725e89d30a9f97c0652629933a3685", "Build")}
    )
