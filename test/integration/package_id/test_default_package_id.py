...
@pytest.mark.parametrize(
    "typedep, typeconsumer, different_id",
    [
...
def test_default_package_id_options(typedep, typeconsumer, different_id):
    ...
    dep = (
        GenConanfile("dep", "0.1")
        .with_option("myopt", [True, False])
        .with_package_type(typedep)
        .with_class_attribute('implements = ["auto_shared_fpic", "auto_header_only"]')
    )
    consumer = (
...
@pytest.mark.parametrize(
    "typedep, versiondep, typeconsumer, different_id",
    [
