...
def test_config_package_id(config_version, mode, result):
    ...
    rrev = info["Local Cache"]["pkg/0.1"]["revisions"][
        "485dad6cb11e2fa99d9afbe44a57a164"
    ]
    package_id = {
        "myconfig/1.2.Z": "c78b4d8224154390356fe04fe598d67aec930199",
...
def test_error_config_package_id():
    ...
    assert (
        "ERROR: core.package_id:config_mode defined, "
...
def test_config_package_id_clear(config_version, mode, result):
    ...
    c.save(
        {
            "conanfile.py": GenConanfile("pkg", "0.1").with_package_id(
                "self.info.clear()"
            )
        }
    )
...
def test_recipe_revision_mode():
    ...
    clientb.save(
        {
            "conanfile.py": GenConanfile("libb", "0.1").with_require(
    ...
    clientc.save(
        {
            "conanfile.py": GenConanfile("libc", "0.1").with_require(
