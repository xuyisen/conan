...
class TestCacheCompatibles:
    ...
    def test_compatible_build_type(self, client):
        client.save(
            {
                "dep/conanfile.py": GenConanfile("dep", "0.1").with_setting(
                    "build_type"
                ),
        ...
        assert (
            "dep/0.1: Main binary package '9e186f6d94c008b544af1569d1a6368d8339efc5' missing"
            in client.out
        )
    ...
    def test_compatible_recipe_reference(self, client):
        ...
        client.save(
            {
                "pkg/conanfile.py": GenConanfile("pkg", "0.1").with_setting(
                    "build_type"
                ),
...
def test_cppstd():
    ...
    base_settings = (
        "-s compiler=gcc -s compiler.version=7 -s compiler.libcxx=libstdc++11"
    )
    client.run(
        f"create dep {base_settings} -s build_type=Release -s compiler.cppstd=14"
    )
    package_id = client.created_package_id("dep/0.1")
    ...
    assert (
        "dep/0.1: Main binary package 'ec174bec4a5ee2d44d3e33d9f4fdacd9b65a6772' missing"
        in client.out
    )
    assert f"Found compatible package '{package_id}'" in client.out

    client.run(
        f"install consumer {base_settings} -s build_type=Debug -s compiler.cppstd=17"
    )
    assert (
        "dep/0.1: Main binary package '94758b7bbcb365aaf355913b35431c0da6ed6da5' missing"
        in client.out
    )
    assert f"Found compatible package '{package_id}'" in client.out
...
def test_cppstd_validated():
    ...
    base_settings = (
        "-s compiler=gcc -s compiler.version=8 -s compiler.libcxx=libstdc++11"
    )
    client.run(f"create dep {base_settings} -s compiler.cppstd=20")

    client.run(
        f"install consumer {base_settings} -s compiler.cppstd=17", assert_error=True
    )
    assert (
        "dep/0.1: Invalid: Current cppstd (17) is lower than the required C++ standard (20)."
        in client.out
    )
...
def test_cppstd_server():
    ...
    assert (
        "dep/0.1: Compatible configurations not found in cache, checking servers"
        in c.out
    )
    assert (
        "dep/0.1: Main binary package '6179018ccb6b15e6443829bf3640e25f2718b931' missing"
        in c.out
    )
    assert (
        "Found compatible package '326c500588d969f55133fdda29506ef61ef03eee': "
        "compiler.cppstd=20" in c.out
    )
    c.assert_listed_binary(
        {"dep/0.1": ("326c500588d969f55133fdda29506ef61ef03eee", "Download (default)")}
    )
    ...
    c.assert_listed_binary(
        {"dep/0.1": ("326c500588d969f55133fdda29506ef61ef03eee", "Cache")}
    )
    ...
    assert (
        "Found compatible package 'ce92fac7c26ace631e30875ddbb3a58a190eb601': "
        "compiler.cppstd=14" in c.out
    )
    c.assert_listed_binary(
        {"dep/0.1": ("ce92fac7c26ace631e30875ddbb3a58a190eb601", "Download (default)")}
    )
...
class TestDefaultCompat:
    def test_default_cppstd_compatibility(self):
        ...
        runtime = "dynamic"
        c.run(
            f"create . -s os={os_} -s arch={arch} -s build_type={build_type} "
        ...
        c.run(
            f"install --requires=mylib/1.0@ -s os={os_} -s arch={arch} -s build_type={build_type} "
            f"-s compiler={compiler} "
            f"-s compiler.version={version} -s compiler.cppstd=14 "
            f"-s compiler.runtime={runtime} -pr:b=profile_build"
        )
        assert (
            "mylib/1.0: Main binary package 'e340edd75790e7156c595edebd3d98b10a2e091e' missing."
    ...
    def test_header_only_build_missing(self):
        ...
        client.assert_listed_binary(
            {"sum/0.1": ("da39a3ee5e6b4b0d3255bfef95601890afd80709", "Build")}
        )
        ...
        client.assert_listed_binary(
            {"sum/0.1": ("da39a3ee5e6b4b0d3255bfef95601890afd80709", "Cache")}
        )
        assert "Installing packages" in client.out
    ...
    def test_check_min_cppstd(self):
        ...
        c.save({"conanfile.py": conanfile})
        settings = (
            "-s compiler=gcc -s compiler.version=9 -s compiler.libcxx=libstdc++11"
        )
        c.run(f"create .  {settings} -s compiler.cppstd=17")
        assert "pkg/0.1: valid standard!!" in c.out
        assert "pkg/0.1: CPPSTD: 17" in c.out
        c.run(
            f"install {settings} --requires=pkg/0.1 -s compiler.cppstd=14",
            assert_error=True,
        )
        assert (
            "pkg/0.1: Invalid: Current cppstd (14) is lower than the required C++ standard (17)."
            in c.out
        )
    ...
    def test_check_min_cstd(self):
        ...
        c.save({"conanfile.py": conanfile})
        settings = (
            "-s compiler=gcc -s compiler.version=14 -s compiler.libcxx=libstdc++11"
        )
        c.run(f"create .  {settings} -s compiler.cstd=17")
        assert "pkg/0.1: valid standard!!" in c.out
        assert "pkg/0.1: CSTD: 17" in c.out
        c.run(
            f"install {settings} --requires=pkg/0.1 -s compiler.cstd=11",
            assert_error=True,
        )
        assert (
            "pkg/0.1: Invalid: Current cstd (11) is lower than the required C standard (17)."
            in c.out
        )
    ...
    def test_can_create_multiple(self):
        ...
        c.save(
            {
                "conanfile.py": GenConanfile("pkg", "0.1").with_settings(
                    "os", "arch", "compiler", "build_type"
                )
            }
        )
        settings = (
            "-s os=Linux -s arch=x86_64 -s compiler=gcc -s compiler.version=9 "
            "-s compiler.libcxx=libstdc++11"
        )
        c.run(f"create . {settings} -s compiler.cppstd=11")
        c.assert_listed_binary(
            {"pkg/0.1": ("0d5f0b9d89187b4e62abb10ae409997e152db9de", "Build")}
        )
        c.run(f"create . {settings} -s compiler.cppstd=14")
        c.assert_listed_binary(
            {"pkg/0.1": ("145f423d315bee340546093be5b333ef5238668e", "Build")}
        )
        c.run(f"create . {settings} -s compiler.cppstd=17")
        c.assert_listed_binary(
            {"pkg/0.1": ("00fcbc3b6ab76a68f15e7e750e8081d57a6f5812", "Build")}
        )
        ...
        c.save(
            {
                ...
                "dep/conanfile.py": GenConanfile("dep", "0.1").with_tool_requires(
                    "tool/0.1"
                ),
    ...
    def test_msvc_194_fallback(self):
        ...
        c.save(
            {
                "conanfile.py": GenConanfile("mylib", "1.0").with_settings(
                    "os", "arch", "compiler", "build_type"
                ),
                "profile_build": "[settings]\nos=Windows\narch=x86_64",
            }
        ...
        c.run(
            "create . -s os=Windows -s arch=x86_64 -s build_type=Release "
            "-s compiler=msvc "
        ...
        # Try to install with cppstd 14, it will find cppstd 17 as compatible
        c.run(
            "install --requires=mylib/1.0@ -s os=Windows -s arch=x86_64 -s build_type=Release "
            "-s compiler=msvc "
...
class TestErrorsCompatibility:
    ...
    def test_error_compatibility(self):
        ...
        assert (
            "Error while processing 'compatibility.py' plugin for 'dep/0.1', line 3"
            in c.out
        )
    ...
    def test_remove_plugin_file(self):
        ...
        os.remove(
            os.path.join(
                HomePaths(c.cache_folder).compatibility_plugin_path, "compatibility.py"
            )
        )
