...
class TestPackageIDRequirementsModes:
    @pytest.mark.parametrize(
        "mode, accepted_version, rejected_version, pattern",
        [
            ("unrelated_mode", "2.0", "", ""),
...
def test_modes(mode, pkg_id):
    ...
    c.save(
        {
            "dep/conanfile.py": GenConanfile("dep", "0.1.1.1").with_settings("os"),
            "pkg/conanfile.py": GenConanfile("pkg", "0.1")
            .with_requires("dep/[*]")
            .with_package_id(package_id_text),
        }
...
class TestPackageIDError:
    def test_transitive_multi_mode_package_id(self):
        ...
        client.save(
            {"conanfile.py": GenConanfile().with_require("dep1/1.0@user/testing")}
        )
        ...
        client.save(
            {
                ...
                .with_require("dep1/1.0@user/testing")
        ...
        client.save(
            {
                "conanfile.py": GenConanfile()
                .with_require("dep2/1.0@user/testing")
                .with_require("dep3/1.0@user/testing")
            }
        )
        client.run(
            "create . --name=consumer --version=1.0 --user=user --channel=testing --build=*"
        )
...
class TestRequirementPackageId:
    ...
    @pytest.mark.parametrize(
        "mode, pattern",
        [("patch_mode", "1.2.3"), ("minor_mode", "1.2.Z"), ("major_mode", "1.Y.Z")],
    )
    def test(self, mode, pattern):
        c = TestClient(light=True)
        pkg = GenConanfile("pkg", "0.1").with_requirement(
            "dep/1.2.3", package_id_mode=mode
        )
        c.save(
            {"dep/conanfile.py": GenConanfile("dep", "1.2.3"), "pkg/conanfile.py": pkg}
        )
        c.run("create dep")
        c.run("create pkg")
        c.run("list pkg:*")
    ...
    def test_wrong_mode(self):
        c = TestClient(light=True)
        pkg = GenConanfile("pkg", "0.1").with_requirement(
            "dep/1.2.3", package_id_mode="nothing"
        )
        c.save(
            {"dep/conanfile.py": GenConanfile("dep", "1.2.3"), "pkg/conanfile.py": pkg}
        )
        ...
        assert (
            "ERROR: require dep/1.2.3 package_id_mode='nothing' is not a known package_id_mode"
            in c.out
        )
    ...
    def test_half_diamond(self, mode, pattern):
        ...
        c.save(
            {
                "liba/conanfile.py": GenConanfile("liba", "1.2.3"),
                "libb/conanfile.py": GenConanfile("libb", "1.2.3").with_requires(
                    "liba/1.2.3"
                ),
    ...
    def test_half_diamond_conflict(self, mode, pattern):
        ...
        libb = GenConanfile("libb", "1.2.3").with_requirement(
            "liba/1.2.3", package_id_mode="patch_mode"
        )
        c.save(
            {
                "liba/conanfile.py": GenConanfile("liba", "1.2.3"),
